...
The machine-readable source of truth is [`scripts/sqlx-dynamic-allowlist.toml`](../scripts/sqlx-dynamic-allowlist.toml). The guard fails on unallowlisted uses, stale allowlist entries, and expired temporary exceptions.
...
CI and the pre-push hook run the dynamic guard plus the SQLx offline prepare check. See [`docs/sqlx-postgres-workflow.md`](sqlx-postgres-workflow.md) for disposable database setup and cache regeneration.
