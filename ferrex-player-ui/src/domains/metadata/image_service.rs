...
use ferrex_player_foundation::units::ByteSize;
use ferrex_player_metadata::constants::{
    image::{IMAGE_MAX_RETRY_ATTEMPTS, IMAGE_RETRY_THROTTLE},
    memory_usage,
};
...
impl UnifiedImageService {
    ...
    pub fn mark_loaded(
        ...
        if let Some(started) = prev_loading_started {
            ...
            // Treat quick loads as immediate display candidates for telemetry.
    ...
    /// Lock contention is reported as an empty queue so callers can keep rendering.
    ...
    /// Outer bounds for adaptive image loading concurrency.
