...
use crate::state_types::{AuthState, AuthStateStore};
...
use ferrex_player_api::ApiClient;

pub use ferrex_player_api::auth::{
    AutoLoginScope, DeviceAuthStatus, DeviceLoginRequest,
    DeviceTrustPolicyResponse, PinLoginRequest, PinPolicyResponse,
    PlayerAuthResult, SetPinRequest,
};
...
impl AuthManager {
        ...
        R: DeserializeOwned,
    {
        ...
        let mut request = self.api_client.http_client().post(&url).json(body);
    ...
    {
        let url = self.api_client.build_url(path);
        let request = self.api_client.http_client().get(&url);
    ...
    pub async fn change_password(
        ...
        let request = self.api_client.http_client().put(&url).json(
    ...
    pub async fn authenticate_device(
        ...
        let summary = crate::dto::UserListItemDto {
            id: user.id,
...
mod tests {
    use super::*;
    use crate::dto::UserListItemDto;
    use crate::storage::AUTH_CACHE_FILE;
