use crate::{
    ...
    infra::{repository::media_repo::MediaRepo, services::api::ApiService},
    state::State,
};
...
use ferrex_player_repository::{
    PlayerDiskMediaRepoCache, content_hash_u64_from_integrity,
};
...
pub fn install_libraries_bootstrap_payload(
    state: &mut State,
    ...
    let seed = encode_libraries_seed_snapshot(&libraries)
        .map_err(|err| RepositoryError::SerializationError(err.to_string()))?;
    let media_repo = MediaRepo::new(seed).map_err(|err| {
...
pub fn handle_libraries_loaded(
    ...
    match result {
        Ok(payload) => {
            if let Err(err) =
                install_libraries_bootstrap_payload(state, payload)
            {
                log::error!(
