...
use crate::{
    database::repository_ports::library::LibraryRepository,
...
impl LibraryRepository for PostgresLibraryRepository {
    async fn create_library(&self, library: Library) -> Result<LibraryId> {
        ...
        sqlx::query!(
            ...
            library.movie_ref_batch_size.get() as i32,
        )
        .execute(self.pool())
        .await
        .map_err(|e| {
    ...
    async fn get_library(&self, id: LibraryId) -> Result<Option<Library>> {
        ...
        Ok(Some(Library {
            ...
            movie_ref_batch_size: MovieReferenceBatchSize::new(
                row.movie_ref_batch_size as u32,
            )
            .map_err(|e| {
                MediaError::InvalidMedia(format!(
                    "Invalid movie_ref_batch_size for library {}: {}",
                    row.id, e
                ))
            })?,
