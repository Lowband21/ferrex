...
use crate::domain::scan::orchestration::manifest_reconcile::{
    ManifestMediaRepository, manifest_scope_reconciliation_prefix,
};
...
#[async_trait]
impl ManifestMediaRepository for PostgresMediaRepository {
    ...
    async fn mark_available_with_fingerprint(
        ...
        self.mark_available_with_fingerprint_impl(
            library_id,
            path_norm,
            fingerprint,
        )
    ...
    async fn mark_unavailable_by_paths(
        ...
        <Self as FolderDeltaRepository>::mark_unavailable_by_paths(
            self, library_id, paths, reason,
        )
        .await
    }
    ...
    async fn mark_unavailable_by_prefixes(
        ...
        <Self as FolderDeltaRepository>::mark_unavailable_by_prefixes(
            self, library_id, prefixes, reason,
        )
        .await
    }
    ...
    async fn delete_folder_inventory_by_prefixes(
        ...
        <Self as FolderDeltaRepository>::delete_folder_inventory_by_prefixes(
            self, library_id, prefixes,
        )
        .await
    }
}

fn normalized_sql_prefix_root(prefix: &str) -> String {
...
fn sql_child_prefix_pattern(root: &str) -> String {
    if root == "/" {
        "/%".to_string()
    } else {
        format!("{root}/%")
