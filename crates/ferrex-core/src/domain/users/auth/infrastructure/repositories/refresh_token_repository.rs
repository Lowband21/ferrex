...
use crate::domain::users::auth::domain::value_objects::{
    RefreshToken, RevocationReason, SessionScope,
};
...
impl RefreshTokenRepository for PostgresRefreshTokenRepository {
    ...
    async fn get_active_refresh_token(
        ...
        Ok(row
            .map(|row| -> Result<_> {
                let family_id =
                    row.family_id.context("refresh token missing family_id")?;
                ...
                let origin_scope = match row.origin_scope.as_str() {
                    ...
                    "playback" => SessionScope::Playback,
                    other => {
                        return Err(anyhow::anyhow!(format!(
                            "invalid origin_scope: {other}"
                        )));
                    }
                };
