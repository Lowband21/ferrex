...
async fn sorted_and_filtered_indices_compact_around_tombstoned_media(
    ...
    let stale_position_count = sqlx::query_scalar!(
        r#"SELECT COUNT(*)::bigint AS "count!" FROM public.movie_sort_positions WHERE library_id = $1"#,
        library_id.to_uuid()
    )
    .fetch_one(&pool)
    .await?;
    ...
    let available_position_count = sqlx::query_scalar!(
        r#"
    ...
    let rebuilt_positions: Vec<(Uuid, i32)> = rows
        .into_iter()
        .map(|row| (row.movie_id, row.title_pos))
        .collect();
