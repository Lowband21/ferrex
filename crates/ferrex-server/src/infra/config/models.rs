...
use super::{
    rate_limits::RateLimitSource,
    scanner::{ScannerConfig, ScannerConfigSource},
};
use crate::infra::{
    constants::{DEFAULT_PASSWORD_PEPPER, DEFAULT_TOKEN_KEY},
    middleware::{
        HstsConfig as HstsLayerConfig,
        RateLimiterConfig as MiddlewareRateLimiterConfig,
    },
};
...
impl From<&HstsSettings> for HstsLayerConfig {
    fn from(value: &HstsSettings) -> Self {
        let mut config = HstsLayerConfig::default();
        config.max_age = value.max_age;
        config.include_subdomains = value.include_subdomains;
...
pub struct RateLimiterSettings {
    pub config: MiddlewareRateLimiterConfig,
