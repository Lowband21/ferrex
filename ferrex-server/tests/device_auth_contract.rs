...
async fn password_device_login_returns_auth_token_and_persists_metadata(
    ...
    assert_eq!(row.0.as_deref(), Some("android"));
    assert_eq!(row.1.as_deref(), Some("2.3.4"));
    assert_eq!(row.2.as_deref(), Some("living-room-hw"));
    assert!(row.3, "remember_device should be per-device");
    assert!(row.4.is_some(), "trusted_until should be persisted");
    assert_eq!(row.5.as_deref(), Some(public_key.as_str()));
    assert_eq!(row.6.as_deref(), Some("ed25519"));
    assert_eq!(row.7["device_id"], json!(device_id));
