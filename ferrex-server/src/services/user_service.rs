...
impl<'a> UserService<'a> {
    ...
    pub fn validate_password(password: &str, requirements: &PasswordRequirements) -> Result<(), String> {
        ...
        if password.len() < requirements.min_length {
            return Err(format!("Password must be at least {} characters", requirements.min_length));
        }
    ...
    pub async fn create_user(&self, params: CreateUserParams) -> AppResult<User> {
        ...
        if let Ok(Some(_)) = self.state.database.backend()
        ...
        let postgres_db = self.state.database.backend()
            .as_any()
        ...
        postgres_db.create_user(&user, &password_hash)
            .await
            .map_err(|e| match e {
    ...
    pub async fn update_user(&self, user_id: Uuid, params: UpdateUserParams) -> AppResult<User> {
        // Get existing user
        let mut user = self.state.database.backend()
    ...
    pub async fn delete_user(&self, user_id: Uuid, deleted_by: Uuid) -> AppResult<()> {
        ...
        let is_admin = self.state.database.backend()
    ...
    pub async fn assign_role(&self, user_id: Uuid, role_id: Uuid, assigned_by: Uuid) -> AppResult<()> {
        // Verify user exists
        let user = self.state.database.backend()
            .get_user_by_id(user_id)
            .await
            .map_err(|_| AppError::not_found("User not found"))?
            .ok_or_else(|| AppError::not_found("User not found"))?;
        ...
        // Verify role exists
        let roles = self.state.database.backend()
            .get_all_roles()
            .await
        ...
        info!("Role '{}' assigned to user {} by {}", role.name, user.username, assigned_by);
        
        Ok(())
    }
    
    /// Remove a role from a user
    pub async fn remove_role(&self, user_id: Uuid, role_id: Uuid, removed_by: Uuid) -> AppResult<()> {
        ...
        info!("Role {} removed from user {} by {}", role_id, user_id, removed_by);
        
        Ok(())
    }
    
    /// Generate authentication tokens for a user
    pub async fn generate_auth_tokens(&self, user_id: Uuid, device_name: Option<String>) -> AppResult<AuthToken> {
        // Generate tokens
    ...
    pub async fn needs_setup(&self) -> AppResult<bool> {
        let users = self.state.database.backend()
            .get_all_users()
            .await
            .map_err(|e| AppError::internal(format!("Failed to get users: {}", e)))?;
        
        for user in users {
            if let Ok(perms) = self.state.database.backend()
