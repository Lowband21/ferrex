...
use ferrex_core::auth::rate_limit::{
    ...
    RateLimitRule, RateLimiter, TrustedSources, ViolationMetadata, backoff,
};
...
enum ConfigUpdate {
    ...
    DynamicRule { endpoint: String, rule: RateLimitRule },
}
...
impl RedisRateLimiter {
    ...
    fn start_background_tasks(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if removed > 0 {
                    debug!("Rate limiter cache cleanup: removed {} expired entries", removed);
                }
            }
        });
    ...
    async fn update_cache(&self, cache_key: String, decision: RateLimitDecision) {
        ...
        cache_guard.insert(cache_key, CachedDecision {
            decision,
            expires_at,
        });
    }
    ...
    async fn record_violation(&self, key: &RateLimitKey, endpoint: &str) {
        ...
        let counter = metrics.violations_by_endpoint
    ...
    async fn check_and_update(
        ...
        let result = match rule.algorithm {
            ...
            _ => {
                ...
                if count == 1 {
                    conn.expire(&redis_key, rule.window.as_secs() as i64).await
                        .map_err(|e| RateLimitError::BackendError(e.into()))?;
                }
        ...
        let violation_count = if !allowed {
            ...
            let count: u32 = conn.incr(&violation_key, 1).await
                .map_err(|e| RateLimitError::BackendError(e.into()))?;
            conn.expire(&violation_key, 86400).await // 24 hours
                .map_err(|e| RateLimitError::BackendError(e.into()))?;
            count
        } else {
            0
        ...
        let actual_reset_after = if !allowed && rule.exponential_backoff && violation_count > rule.violation_threshold {
            backoff::exponential(rule.backoff_base, violation_count - rule.violation_threshold, rule.max_backoff)
        } else {
            reset_after
        };
    ...
    async fn get_current_state(
        ...
        let current_count: u32 = match rule.algorithm {
            ferrex_core::auth::rate_limit::RateLimitAlgorithm::SlidingWindowLog => {
                ...
                conn.zcount(&redis_key, (now - rule.window.as_secs()).to_string(), "+inf").await
                    .map_err(|e| RateLimitError::BackendError(e.into()))?
            }
            _ => {
...
fn extract_rate_limit_key(request: &Request<Body>) -> RateLimitKey {
    ...
    if let Some(device_id) = request.headers()
        .get("X-Device-ID")
        ...
        Ok(decision) => {
            if decision.allowed {
                ...
                headers.insert(
                    ...
                    (decision.limit - decision.current_count).to_string().parse().unwrap(),
                );
                headers.insert(
                    "X-RateLimit-Reset",
