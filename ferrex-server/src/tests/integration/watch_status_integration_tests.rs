...
mod tests {
    ...
    use ferrex_core::{
        api_types::MediaId,
        media::{MovieID, EpisodeID},
        watch_status::{UpdateProgressRequest, InProgressItem},
        MediaFile,
    };
    ...
    async fn test_update_watch_progress() {
        ...
        let watch_state = state.db.backend().get_user_watch_state(user.id).await.unwrap();
    ...
    async fn test_get_watch_state() {
        ...
        state.db.backend().update_watch_progress(user.id, &UpdateProgressRequest {
    ...
    async fn test_mark_media_completed() {
        ...
        // Verify completion
        let media_id_str = MediaId::Movie(MovieID::new(media_id.to_string()).unwrap());
        let is_completed = state.db.backend().is_media_completed(user.id, &media_id_str).await.unwrap();
        assert!(is_completed);
    }
