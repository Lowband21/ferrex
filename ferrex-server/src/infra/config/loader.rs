...
impl ConfigLoader {
    ...
    fn compose_config(
        ...
        let server = ServerConfig {
            ...
            port: file_server.port.or(env.server_port).unwrap_or(3000),
        };

        let database = DatabaseConfig {
            primary_url: file_database.url.or(env.database_url),
        };

        let redis = file_redis
            .map(|r| RedisConfig { url: r.url })
            .or_else(|| env.redis_url.map(|url| RedisConfig { url }));

        let media = MediaConfig {
            root: file_media.root.or(env.media_root),
        };

        let cache_root = file_cache
            .root
            .or(env.cache_root)
            .unwrap_or_else(|| PathBuf::from("./cache"));
        let transcode = file_cache
            .transcode
            .or(env.cache_transcode)
            .unwrap_or_else(|| cache_root.join("transcode"));
        let thumbnails = file_cache
            .thumbnails
            .or(env.cache_thumbnails)
        ...
        let ffmpeg = FfmpegConfig {
            ffmpeg_path: file_ffmpeg
                .ffmpeg_path
                .or(env.ffmpeg_path)
                .unwrap_or_else(|| "ffmpeg".into()),
            ffprobe_path: file_ffmpeg
                .ffprobe_path
                .or(env.ffprobe_path)
        ...
        let cors = CorsConfig {
            allowed_origins: file_cors
                .allowed_origins
                .or(env.cors_allowed_origins)
                .unwrap_or_else(|| default_cors_origins()),
            allowed_methods: file_cors
                .allowed_methods
                .or(env.cors_allowed_methods)
                .unwrap_or_else(|| default_cors_methods()),
            allowed_headers: file_cors
                .allowed_headers
                .or(env.cors_allowed_headers)
                .unwrap_or_else(|| default_cors_headers()),
            allow_credentials: file_cors
                .allow_credentials
                .or(env.cors_allow_credentials)
                .unwrap_or(false),
        };

        let dev_mode = file_dev_mode.or(env.dev_mode).unwrap_or(false);

        let security = SecurityConfig {
            enforce_https: file_security
                .enforce_https
                .or(env.enforce_https)
                .unwrap_or(false),
            trust_proxy_headers: file_security
                .trust_proxy_headers
                .or(env.trust_proxy_headers)
                .unwrap_or(false),
            hsts: HstsSettings {
                max_age: file_security
                    .hsts
                    .max_age
                    .or(env.hsts_max_age)
                    .unwrap_or(31_536_000),
                include_subdomains: file_security
                    .hsts
                    .include_subdomains
                    .or(env.hsts_include_subdomains)
                    .unwrap_or(false),
                preload: file_security
                    .hsts
                    .preload
                    .or(env.hsts_preload)
                    .unwrap_or(false),
        ...
        let auth = AuthConfig {
            password_pepper: file_auth
                .password_pepper
                .or(env.auth_password_pepper)
                .unwrap_or_else(|| DEFAULT_PASSWORD_PEPPER.to_string()),
            token_key: file_auth
                .token_key
                .or(env.auth_token_key)
                .unwrap_or_else(|| DEFAULT_TOKEN_KEY.to_string()),
            setup_token: file_auth.setup_token.or(env.setup_token),
        };
