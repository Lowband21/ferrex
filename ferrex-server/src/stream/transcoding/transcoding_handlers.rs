...
pub async fn start_transcode_handler(
    ...
    let profile = if let Some(profile_name) = request.profile {
        match profile_name.as_str() {
            "hdr_to_sdr_1080p" => TranscodingProfile::hdr_to_sdr_1080p(),
            "hdr_to_sdr_4k" => TranscodingProfile::hdr_to_sdr_4k(),
            _ => {
                return Ok(Json(json!({
                    "status": "error",
        ...
        TranscodingProfile::hdr_to_sdr_1080p()
    } else {
        return Ok(Json(json!({
            "status": "error",
...
pub async fn transcode_status_handler(
    ...
    if let Some(job) = state.transcoding_service.get_job_status(&job_id).await {
        ...
        let status = match &job.status {
            TranscodingStatus::Pending => ferrex_core::TranscodingStatus::Pending,
            TranscodingStatus::Queued => ferrex_core::TranscodingStatus::Queued,
