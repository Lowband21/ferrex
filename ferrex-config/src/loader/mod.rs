...
use crate::{
    constants::{DEFAULT_PASSWORD_PEPPER, DEFAULT_TOKEN_KEY},
...
impl ConfigLoader {
    ...
    pub fn load(&self) -> Result<ConfigLoad, error::ConfigLoadError> {
    ...
    fn load_file_config(
        ...
        (Option<FileConfig>, Option<PathBuf>, bool),
    ...
    fn compose_config(
    ...
    ) -> Result<(Config, ConfigWarnings), error::ConfigLoadError> {
        ...
        let database = DatabaseConfig {
            primary_url: self.database_url(&env, &file_database)?,
        };
        ...
        let (rate_limiter, rate_limit_source) =
            if let Some(env_spec) = env.rate_limits {
                let (config, source) = env_spec
                    .load_from_env()
                    .map_err(error::ConfigLoadError::RateLimiter)?;
        ...
        config.ensure_directories().map_err(|err| {
            error::ConfigLoadError::Filesystem { source: err }
        })?;
        config.normalize_paths().map_err(|err| {
            error::ConfigLoadError::Filesystem { source: err }
        })?;
