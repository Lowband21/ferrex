...
impl ThumbnailService {
    ...
    pub async fn get_or_extract_thumbnail(&self, media_id: &str) -> Result<PathBuf> {
        ...
        let media = self.db.backend().get_media(media_id).await
...
fn extract_frame_at_percentage(input_path: &str, output_path: &Path, percentage: f64) -> Result<()> {
    tracing::info!("Extracting thumbnail from {} at {}% to {:?}", input_path, percentage * 100.0, output_path);
    ...
    let (time_base, stream_duration, codec_params) = {
        ...
        tracing::debug!("Video stream: duration={}, time_base={}/{}", 
            duration, time_base.numerator(), time_base.denominator());
    ...
    if format_duration > 0 && percentage > 0.0 {
        ...
        tracing::info!("Seeking to {}% = {} microseconds", percentage * 100.0, seek_position);
        ...
        let seek_seconds = (target_ts as f64 * time_base.numerator() as f64) / time_base.denominator() as f64;
        let seek_position = (seek_seconds * 1_000_000.0) as i64;
        
        tracing::info!("Seeking using stream duration to {}% = {} microseconds", percentage * 100.0, seek_position);
    ...
    tracing::info!("Video info: {}x{} (aspect ratio: {:.2})", 
    ...
    for (stream, packet) in input_ctx.packets() {
        if stream.index() == video_stream_index {
            ...
            if packet_count > max_packets {
                tracing::warn!("Processed {} packets without finding a suitable frame", max_packets);
                break;
            }
                ...
                match decoder.receive_frame(&mut decoded_frame) {
                    Ok(_) => {
                        ...
                        if !seek_succeeded && frame_count < 10 {
                            tracing::debug!("Seek failed, skipping frame {} to avoid black frames at start", frame_count);
                            continue;
                        }
                        ...
                        tracing::info!("Frame scaled to {}x{}, format: {:?}", 
                        ...
                        tracing::debug!("Frame data sample: {} non-zero bytes out of {}", non_zero_count, sample_size);
                        
                        if non_zero_count == 0 && !seek_succeeded && frame_count < 50 {
                            tracing::warn!("Frame {} appears to be all black, trying next frame", frame_count);
                            continue;
                        }
    ...
    Err(anyhow::anyhow!("Failed to extract any frame after {} packets", frame_count))
}
...
fn save_frame_as_jpeg(frame: &ffmpeg::util::frame::video::Video, output_path: &Path) -> Result<()> {
    ...
    tracing::debug!("Saving frame: {}x{}, stride: {}, data len: {}", 
