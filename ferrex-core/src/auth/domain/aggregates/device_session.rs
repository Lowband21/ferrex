...
use crate::auth::domain::value_objects::{
    SessionToken, DeviceFingerprint, PinCode, PinPolicy,
};
...
impl DeviceSession {
    ...
    pub fn set_pin(&mut self, pin_value: String, policy: &PinPolicy) -> Result<(), DeviceSessionError> {
        // Create the PIN with validation
        let pin = PinCode::new(pin_value, policy)
    ...
    pub fn authenticate_with_pin(
        ...
        let pin = self.pin.as_ref()
            .ok_or(DeviceSessionError::PinNotSet)?;
        ...
        // Verify PIN
        let valid = pin.verify(pin_value)
            .map_err(|_| DeviceSessionError::InvalidPin)?;
    ...
    pub fn refresh_token(&mut self, session_lifetime: Duration) -> Result<SessionToken, DeviceSessionError> {
        // Check device status
        ...
        let current_token = self.session_token.as_ref()
    ...
    pub fn device_fingerprint(&self) -> &DeviceFingerprint { &self.device_fingerprint }
    pub fn device_name(&self) -> &str { &self.device_name }
    pub fn status(&self) -> DeviceStatus { self.status }
    pub fn has_pin(&self) -> bool { self.pin.is_some() }
    pub fn failed_attempts(&self) -> u8 { self.failed_attempts }
    pub fn created_at(&self) -> DateTime<Utc> { self.created_at }
    pub fn last_activity(&self) -> DateTime<Utc> { self.last_activity }
    pub fn session_token(&self) -> Option<&SessionToken> { self.session_token.as_ref() }
}
...
mod tests {
    ...
    fn test_device_lifecycle() {
        ...
        // Authenticate
        let token = session.authenticate_with_pin("5823", 3, Duration::hours(1)).unwrap();
