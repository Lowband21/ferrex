...
use crate::auth::domain::repositories::{UserAuthenticationRepository, DeviceSessionRepository};
use crate::auth::domain::aggregates::{UserAuthentication, DeviceSession, DeviceSessionError};
use crate::auth::domain::value_objects::{SessionToken, DeviceFingerprint, PinPolicy};
...
impl AuthenticationService {
    ...
    pub async fn revoke_device(
        ...
        session.revoke().map_err(|_| AuthenticationError::InvalidCredentials)?;
    ...
    pub async fn verify_session(
        ...
        match session.session_token() {
            Some(stored_token) if stored_token.secure_compare(token.as_str()) && stored_token.is_valid() => {
                Ok(session)
            }
