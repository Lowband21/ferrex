...
impl MediaDatabase {
    ...
    pub async fn query_media_with_cache(
        ...
        if let Some(cache) = &self.cache {
            ...
            match cache_conn.get::<Vec<crate::query::MediaReferenceWithStatus>>(&cache_key).await {
    ...
    pub async fn invalidate_library_query_cache(&self, library_id: uuid::Uuid) -> Result<()> {
        use cache::CacheKeys;
        use tracing::{info, warn};
    ...
    pub async fn store_movie_reference_with_cache_invalidation(
        &self,
        movie: &crate::media::MovieReference
    ) -> Result<()> {
    ...
    pub async fn store_series_reference_with_cache_invalidation(
        &self,
        series: &crate::media::SeriesReference
    ) -> Result<()> {
        use crate::database::traits::MediaDatabaseTrait;
