...
impl<'a> FilteredMovieIndexBuilder<'a> {
    ...
    fn build(
        mut self,
    ) -> std::result::Result<QueryBuilder<'a, Postgres>, FilterQueryError> {
        self.apply_filters()?;
        self.apply_sort()?;
        Ok(self.qb)
    }
    ...
    fn apply_watch_status_filter(
        ...
        match filter {
            ...
            WatchStatusFilter::Unwatched => {
                ...
                self.qb.push(
                    " AND uwp.media_uuid IS NULL AND ucm.media_uuid IS NULL",
                );
            }
...
fn map_sort_to_column(
    ...
    match (sort, order) {
        ...
        (SortBy::Rating, SortOrder::Descending) => {
            ("msp.rating_pos_desc", "ASC")
        }
        (SortBy::Runtime, SortOrder::Ascending) => ("msp.runtime_pos", "ASC"),
        (SortBy::Runtime, SortOrder::Descending) => {
            ("msp.runtime_pos_desc", "ASC")
        }
        (SortBy::Popularity, SortOrder::Ascending) => {
            ("msp.popularity_pos", "ASC")
        }
        (SortBy::Popularity, SortOrder::Descending) => {
            ("msp.popularity_pos_desc", "ASC")
        }
        (SortBy::Bitrate, SortOrder::Ascending) => ("msp.bitrate_pos", "ASC"),
        (SortBy::Bitrate, SortOrder::Descending) => {
            ("msp.bitrate_pos_desc", "ASC")
        }
        (SortBy::FileSize, SortOrder::Ascending) => {
            ("msp.file_size_pos", "ASC")
        }
        (SortBy::FileSize, SortOrder::Descending) => {
            ("msp.file_size_pos_desc", "ASC")
        }
        ...
        (SortBy::Resolution, SortOrder::Ascending) => {
            ("msp.resolution_pos", "ASC")
        }
