...
impl QueryRepository for PostgresQueryRepository {
    ...
    async fn query_in_progress_media(
        ...
        let rows = sqlx::query(
            r#"
            WITH inprog AS (
                SELECT media_uuid, image_type, position, duration, last_watched
            ...
            (
                SELECT
                    ...
                    NULL::INT2         AS episode_number,
                    NULL::INT2         AS season_number,
                    NULL::BIGINT       AS tmdb_series_id,
                    NULL::TIMESTAMPTZ  AS episode_discovered_at,
                    NULL::TIMESTAMPTZ  AS episode_created_at,
                    inprog.position,
                    inprog.duration,
                    inprog.last_watched,
                    0                  AS media_kind
                FROM inprog
                JOIN movie_references mr ON inprog.media_uuid = mr.id AND inprog.image_type = 0
                JOIN media_files mf ON mr.file_id = mf.id
                ...
                SELECT
                    ...
                    3                  AS media_kind
                FROM inprog
                JOIN episode_references er ON inprog.media_uuid = er.id AND inprog.image_type = 3
        ...
        for row in rows {
            let kind: i32 = row.get("media_kind");
            let media = if kind == 0 {
                Media::Movie(self.row_to_movie_reference(&row)?)
            } else {
                Media::Episode(self.row_to_episode_reference(&row)?)
            };

            let watch_status = InProgressItem {
                media_id: row.get("file_id"),
    ...
    async fn query_completed_media(
        ...
        let rows = sqlx::query(
            r#"
            WITH completed AS (
                SELECT media_uuid, image_type, completed_at
            ...
            (
                SELECT
                    ...
                    NULL::TIMESTAMPTZ  AS episode_discovered_at,
                    NULL::TIMESTAMPTZ  AS episode_created_at,
                    completed.completed_at AS last_watched,
                    0                  AS media_kind
                FROM completed
                JOIN movie_references mr ON completed.media_uuid = mr.id AND completed.image_type = 0
                JOIN media_files mf ON mr.file_id = mf.id
                ...
                SELECT
                    ...
                    3                  AS media_kind
                FROM completed
                JOIN episode_references er ON completed.media_uuid = er.id AND completed.image_type = 3
                JOIN media_files mf ON er.file_id = mf.id
            )
            ORDER BY last_watched DESC
            LIMIT $2 OFFSET $3
    ...
    fn row_to_episode_reference(
        &self,
        row: &sqlx::postgres::PgRow,
    ) -> Result<EpisodeReference> {
        // Hydrate MediaFile from joined columns
        let technical_metadata: Option<serde_json::Value> =
            row.try_get("technical_metadata").ok();
        let media_file_metadata = technical_metadata
            .map(serde_json::from_value)
        ...
        let episode_id: Uuid = row.get("id");
        let season_id: Uuid = row.get("season_id");
        ...
        let tmdb_series_id: i64 = row.get("tmdb_series_id");
