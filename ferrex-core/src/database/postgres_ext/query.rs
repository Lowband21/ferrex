...
impl PostgresDatabase {
    ...
    async fn query_media_legacy(
        ...
        let mut watch_state = if let Some(user_id) = query.user_context {
            ...
            if let Some(ref state) = watch_state {
                match watch_filter {
                    crate::watch_status::WatchStatusFilter::InProgress => {
                        ...
                        for (media_id, progress_item) in &state.in_progress {
                            ...
                            results.push(MediaReferenceWithStatus {
                                media: media_ref,
                                watch_status: Some(progress_item.clone()),
                                is_completed: false,
                            });
                        }
                    }
                ...
                match library_ref.library_type {
                    ...
                    crate::LibraryType::Movies => {
                        ...
                        for movie in movies {
                            ...
                            if let Some(watch_status) = watch_state.clone().as_mut().map(|state| {
                                state.in_progress.get(&MediaId::Movie(movie_id).to_owned())
                            }) {
                                let is_completed = watch_state
                                    .clone()
                                    .as_ref()
                                    .map(|state| {
                                        state.completed.contains(&MediaId::Movie(movie_id.clone()))
                                    })
                                    .unwrap_or(false);

                                results.push(MediaReferenceWithStatus {
                                    media: MediaReference::Movie(movie),
                                    watch_status: watch_status.map(|status| status.clone()),
                                    is_completed,
                                });
                            }
                        }
                                ...
                                for season in seasons {
                                    ...
                                    if let Ok(episodes) = self.get_season_episodes(&season_id).await
                                    {
                                        for episode in episodes {
                                            ...
                                            if let Some((Some(progress), Some(is_completed))) =
                                                watch_state.clone().as_mut().map(|state| {
                                                    let progress = state.in_progress.get(
                                                        &MediaId::Episode(episode_id).to_owned(),
                                                    );
                                                    let is_completed =
                                                        watch_state.as_ref().map(|state| {
                                                            state.completed.contains(
                                                                &MediaId::Episode(
                                                                    episode_id.clone(),
                                                                ),
                                                            )
                                                ...
                                                })
                                            {
                                                results.push(MediaReferenceWithStatus {
                                                    ...
                                                    watch_status: Some(progress.clone()),
                                                    is_completed,
                                                });
                                            };
                                        }
                ...
                results.sort_by(|a, b| {
                    ...
                    let a_sort_key = match &a.media {
                        ...
                        MediaReference::Episode(e) => (
                            "",
                            e.season_number.value() as i32,
                            e.episode_number.value() as i32,
                        ),
                    };
