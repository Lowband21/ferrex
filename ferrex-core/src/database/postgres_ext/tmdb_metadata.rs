...
impl<'a> TmdbMetadataRepository<'a> {
    ...
    pub async fn load_movie_reference(
        ...
        Ok(MovieReference {
            ...
            details: details
                .map(|d| {
                    MediaDetailsOption::Details(Box::new(TmdbDetails::Movie(d)))
                })
                .unwrap_or_else(|| {
    ...
    pub async fn load_series_reference(
        ...
        Ok(SeriesReference {
            ...
            details: details
                .map(|d| {
                    MediaDetailsOption::Details(Box::new(TmdbDetails::Series(
                        d,
                    )))
                })
                .unwrap_or_else(|| {
    ...
    pub async fn load_season_reference(
        ...
        let details_option = details
            .map(|d| {
                MediaDetailsOption::Details(Box::new(TmdbDetails::Season(d)))
            })
            .unwrap_or_else(|| {
    ...
    pub async fn load_episode_reference(
        ...
        let details_option = details
            .map(|d| {
                MediaDetailsOption::Details(Box::new(TmdbDetails::Episode(d)))
            })
            .unwrap_or_else(|| {
...
async fn upsert_series_reference(
    ...
    let result_id = if let Some(tmdb_value) = tmdb_id {
        ...
        .map_err(|e: sqlx::Error| {
            MediaError::Internal(format!("Failed to upsert series reference: {}", e))
        })?;
        ...
        .await
        .map_err(|e: sqlx::Error| {
            MediaError::Internal(format!("Failed to upsert local series reference: {}", e))
        })?;
...
async fn load_season_details(
    ...
    let season_number = u8::try_from(raw_season_number).unwrap_or_default();
