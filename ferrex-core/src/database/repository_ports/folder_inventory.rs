...
pub trait FolderInventoryRepository: Send + Sync {
    ...
    /// Delete folder inventory entries whose `folder_path` begins with any of
    /// the provided prefixes.
    ///
    /// Callers should pass canonical, library-scoped folder roots. Implementations must delete
    /// both the root itself (`folder_path = '{root}'`) and any children under it
    /// (`folder_path LIKE '{root}/%'`).
