...
use ferrex_model::{ImageMediaType, MediaID, media_type::VideoMediaType};
...
use crate::{
    database::{
        repository_ports::processing_status::ProcessingStatusRepositoryTrait,
...
impl PostgresProcessingStatusRepository {
    ...
    pub async fn fetch_unprocessed(
        ...
        match status_type {
            "metadata" => {
                let rows = sqlx::query!(
                    r#"
                    SELECT f.id, f.media_id, f.media_type AS "media_type!: VideoMediaType", f.library_id, f.file_path, f.filename, f.file_size,
                ...
                for row in rows {
                    ...
                    let media_id: MediaID =
                        MediaID::from((row.media_id, row.media_type));
    ...
    pub async fn fetch_failed(
        ...
        let rows = sqlx::query!(
            r#"
            SELECT f.id, f.media_id, f.media_type AS "media_type!: ImageMediaType", f.library_id, f.file_path, f.filename, f.file_size,
                   f.technical_metadata, f.parsed_info, f.discovered_at, f.created_at, f.updated_at
            FROM media_files f
        ...
        for row in rows {
            ...
            // Only movie and episode types are valid for the media_files table
            let media_id: MediaID =
                if matches!(row.media_type, ImageMediaType::Movie) {
                    MediaID::from((row.media_id, VideoMediaType::Movie))
                } else {
                    MediaID::from((row.media_id, VideoMediaType::Episode))
                };
...
#[async_trait]
impl ProcessingStatusRepositoryTrait for PostgresProcessingStatusRepository {
