use crate::{
    ...
    LibraryReference, MediaDatabase, Result, ScanOutput,
};
use std::sync::Arc;
...
impl BackgroundScanner {
    ...
    pub async fn run(self: Arc<Self>) -> Result<()> {
        ...
        for library in &libraries {
            if let Ok(lib) = self.db.backend().get_library(&library.id.to_string()).await {
                if let Some(lib) = lib {
                    if lib.enabled && lib.watch_for_changes {
                        if let Err(e) = self.file_watcher.watch_library(library).await {
                            error!("Failed to start file watcher for library {}: {}", library.name, e);
                        }
                    }
                }
            }
            ...
            match self.db.backend().list_library_references().await {
                Ok(libraries) => {
                    for library in libraries {
                        ...
                        match self.file_watcher.get_unprocessed_events(library.id, 100).await {
                            Ok(events) => {
                                for event in events {
                                    ...
                                    match event.event_type {
                                        FileWatchEventType::Created | FileWatchEventType::Modified => {
                                            ...
                                            if let Err(e) = self.scan_media_folder(&library, &event.file_path).await {
                                                error!("Failed to scan media folder for {}: {}", event.file_path, e);
                                            }
                                        }
                                        FileWatchEventType::Deleted => {
                                            // Remove from database
                                            if let Err(e) = self.handle_file_deletion(&event.file_path).await {
                                                error!("Failed to handle file deletion {}: {}", event.file_path, e);
                                            }
                                        }
                            ...
                            Err(e) => {
                                error!("Failed to get unprocessed events for library {}: {}", library.id, e);
                            }
                        }
                    }
                }
                ...
                Ok(libraries) => {
                    for library in libraries {
                        ...
                        if library.needs_scan() {
                            ...
                            match self.orchestrator.create_scan(&library_ref, ScanType::Incremental, scan_options.clone()).await {
                                Ok(scan_state) => {
                                    ...
                                    // Run the scan
                                    tokio::spawn(async move {
                                        if let Err(e) = scanner.scan_incremental(library_ref, &scan_state, tx).await {
                                ...
                                Err(e) => {
                                    error!("Failed to create scan for library {}: {}", library.name, e);
                                }
                            }
                        }
                    }
    ...
    async fn scan_media_folder(&self, library: &LibraryReference, file_path: &str) -> Result<()> {
        ...
        info!("Scanning media folder: {} (library type: {:?})", 
            folder.display(), library.library_type);
            ...
            crate::LibraryType::Movies => {
                match scanner.process_movie_folder(folder, library.id).await {
                    ...
                    Err(e) => {
                        ...
                        let _ = tx.send(ScanOutput::Error {
    ...
    async fn handle_file_deletion(&self, file_path: &str) -> Result<()> {
        ...
        if let Some(media_file) = self.db.backend().get_media_by_path(file_path).await? {
            ...
            self.db.backend().delete_media(&media_file.id.to_string()).await?;
