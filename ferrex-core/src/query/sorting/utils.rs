...
pub fn reorder_by_indices<T: Clone>(items: &mut [T], indices: &[usize]) {
    debug_assert_eq!(items.len(), indices.len(), "Indices length must match items length");
    ...
    F: FnMut(&T, &T) -> std::cmp::Ordering,
{
    items.windows(2).all(|w| {
        matches!(compare(&w[0], &w[1]), std::cmp::Ordering::Less | std::cmp::Ordering::Equal)
    })
}
...
mod tests {
    ...
    fn test_calculate_items_hash() {
        ...
        assert_ne!(hash1, hash3, "Different items should produce different hash");
    }
}