use crate::{MediaError, Result, query::{MediaQuery, decision_engine::{QueryComplexityAnalyzer, QueryComplexity}}};
...
impl QueryComplexityGuard {
    ...
    pub fn check_query(&self, query: &MediaQuery) -> Result<()> {
        ...
        if complexity_score > max_allowed {
            warn!(
                ...
                complexity_score, max_allowed, self.is_peak_hours()
            );
    ...
    fn is_peak_hours(&self) -> bool {
        ...
        if self.config.peak_hours_start <= self.config.peak_hours_end {
            ...
            current_hour >= self.config.peak_hours_start && current_hour < self.config.peak_hours_end
        } else {
    ...
    pub fn suggest_simplification(&self, query: &MediaQuery) -> Vec<String> {
        ...
        if let Some(search) = &query.search {
            if search.fuzzy && search.fields.len() > 1 {
                suggestions.push("Consider searching only in specific fields instead of all fields".to_string());
            }
            if search.fuzzy {
        ...
        if query.pagination.offset > 1000 {
            suggestions.push("Consider using cursor-based pagination for large offsets".to_string());
        }
        
        if query.filters.watch_status.is_some() && !query.filters.library_ids.is_empty() {
            suggestions.push("Consider removing library filter when using watch status filter".to_string());
        }
...
mod tests {
    ...
    fn test_complex_query_rejected() {
        ...
        let query = MediaQuery {
            filters: MediaFilters {
                genres: vec!["Action".to_string(), "Drama".to_string(), "Comedy".to_string()],
    ...
    fn test_simplification_suggestions() {
        ...
        let query = MediaQuery {
            filters: MediaFilters {
                genres: vec!["A".to_string(), "B".to_string(), "C".to_string(), "D".to_string()],
                ..Default::default()
            },
            sort: SortCriteria {
                primary: SortField::Title,
        ...
        assert!(suggestions.iter().any(|s| s.contains("cursor-based pagination")));
    }
}