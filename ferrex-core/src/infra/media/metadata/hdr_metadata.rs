...
impl HdrMetadataExtractor {
    ...
    pub fn extract_hdr_metadata<P: AsRef<Path>>(
        file_path: P,
    ) -> Result<HdrInfo> {
        ...
        // Parse JSON output
        let json_str = std::str::from_utf8(&output.stdout).map_err(|_| {
            MediaError::InvalidMedia("Invalid ffprobe output".to_string())
        })?;

        let json: serde_json::Value =
            serde_json::from_str(json_str).map_err(|_| {
                MediaError::InvalidMedia(
                    "Failed to parse ffprobe JSON".to_string(),
                )
            })?;
