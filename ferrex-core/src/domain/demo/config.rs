use crate::domain::demo::policy::DemoPolicy;
...
pub struct DemoSeedOptions {
    /// Optional explicit root directory. When omitted the server chooses a
    /// default demo root (typically `<cache_root>/demo-media`).
    ...
    /// Skip generating episode thumbnails from demo files (they are synthetic).
    pub skip_episode_thumbnails: bool,
...
impl DemoSeedOptions {
    ...
    pub fn from_env() -> Self {
        ...
        if let Ok(path) = std::env::var("FERREX_DEMO_ROOT")
            && let Some(trimmed) = normalize_env_override(&path)
        {
            opts.root = Some(PathBuf::from(trimmed));
        }

        if let Ok(flag) = std::env::var("FERREX_DEMO_ALLOW_DEVIATIONS")
            && let Some(parsed) = parse_env_bool(&flag)
        {
            opts.allow_deviations = parsed;
        }
        ...
        if let Ok(flag) = std::env::var("FERREX_DEMO_SKIP_METADATA")
            && let Some(parsed) = parse_env_bool(&flag)
        {
            opts.skip_metadata_probe = parsed;
        }

        if let Ok(flag) = std::env::var("FERREX_DEMO_SKIP_THUMBNAILS")
            && let Some(parsed) = parse_env_bool(&flag)
        {
            opts.skip_episode_thumbnails = parsed;
        }

        if let Ok(flag) = std::env::var("FERREX_DEMO_ZERO_LENGTH")
            && let Some(parsed) = parse_env_bool(&flag)
        {
            opts.allow_zero_length_files = parsed;
        }
    ...
    pub fn policy(&self) -> DemoPolicy {
        DemoPolicy {
            ...
            skip_episode_thumbnails: self.skip_episode_thumbnails,
        }
    }
}
...
fn parse_env_bool(value: &str) -> Option<bool> {
    ...
    if matches_ignore_ascii_case(value, ["1", "true", "yes"]) {
        return Some(true);
    }

    if matches_ignore_ascii_case(value, ["0", "false", "no"]) {
        return Some(false);
    }
