...
impl RedisCache {
    pub async fn new(redis_url: &str) -> Result<Self> {
        ...
        let conn = ConnectionManager::new(client).await.map_err(|e| {
            MediaError::InvalidMedia(format!("Failed to connect to Redis: {e}"))
        })?;
    ...
    pub async fn get<T: DeserializeOwned>(
        ...
        let data: Option<String> = self.conn.get(key).await.map_err(|e| {
            MediaError::InvalidMedia(format!("Redis GET failed: {e}"))
        })?;
        ...
        match data {
            Some(json) => {
                let value = serde_json::from_str(&json).map_err(|e| {
                    MediaError::InvalidMedia(format!(
                        "Failed to deserialize cache data: {e}"
                    ))
                })?;
                debug!("Cache HIT: {}", key);
                Ok(Some(value))
    ...
    pub async fn set<T: Serialize>(
        ...
        if let Some(ttl) = ttl {
            self.conn
                ...
                .await
                .map_err(|e| {
                    MediaError::InvalidMedia(format!("Redis SETEX failed: {e}"))
                })?;
        } else {
            self.conn.set::<_, _, ()>(key, json).await.map_err(|e| {
                MediaError::InvalidMedia(format!("Redis SET failed: {e}"))
            })?;
        }
    ...
    pub async fn delete(&mut self, key: &str) -> Result<()> {
        ...
        self.conn.del::<_, ()>(key).await.map_err(|e| {
            MediaError::InvalidMedia(format!("Redis DEL failed: {e}"))
        })?;
    ...
    pub async fn delete_pattern(&mut self, pattern: &str) -> Result<()> {
        ...
        let keys: Vec<String> = self.conn.keys(pattern).await.map_err(|e| {
            MediaError::InvalidMedia(format!("Redis KEYS failed: {e}"))
        })?;

        if !keys.is_empty() {
            debug!(
                "Deleting {} keys matching pattern: {}",
                keys.len(),
                pattern
            );
            let _: () = self.conn.del(keys).await.map_err(|e| {
                MediaError::InvalidMedia(format!("Redis DEL failed: {e}"))
            })?;
        }
    ...
    pub async fn flush_all(&mut self) -> Result<()> {
        ...
        redis::cmd("FLUSHDB")
            ...
            .await
            .map_err(|e| {
                MediaError::InvalidMedia(format!("Redis FLUSHDB failed: {e}"))
            })?;
