...
use ferrex_player_app::app::bootstrap::{AppConfig, base_state};
use ferrex_player_app::domains::auth::messages as auth_msgs;
use ferrex_player_app::domains::auth::types::AuthenticationFlow;
...
async fn empty_users_plus_setup_failure_transitions_to_first_run() {
    ...
    let _ = ferrex_player_app::domains::auth::update::update_auth(
        &mut state,
    ...
    let setup_status = ferrex_player_app::infra::api_client::SetupStatus {
        needs_setup: true,
        has_admin: false,
