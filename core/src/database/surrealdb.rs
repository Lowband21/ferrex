...
impl MediaDatabaseTrait for SurrealDatabase {
    ...
    async fn store_external_metadata(&self, _media_id: &str, _metadata: &MediaMetadata) -> Result<()> {
    ...
    async fn store_tv_show(&self, _show_info: &TvShowInfo) -> Result<String> {
        ...
        Err(MediaError::InvalidMedia("TV show storage not implemented for SurrealDB".to_string()))
    }
