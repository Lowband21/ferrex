...
impl HlsClient {
    ...
    pub async fn start_adaptive_transcoding_with_retry(
        &self,
        media_id: &str,
        max_retries: u32,
            ...
            match self.start_adaptive_transcoding(media_id).await {
                ...
                Err(e) => {
                    ...
                    if retries <= max_retries {
                        log::warn!(
                            "Transcoding start failed, retrying ({}/{}): {}",
                            retries,
                            max_retries,
                            last_error
                        );
    ...
    pub async fn start_adaptive_transcoding(
        ...
        log::info!(
            "Adaptive transcoding started with job ID: {}",
            master_job_id
        );
        Ok(master_job_id)
    }
    ...
    pub async fn fetch_master_playlist_with_retry(
        ...
        while retries <= max_retries {
            match self.fetch_master_playlist(media_id).await {
                ...
                Err(e) => {
                    ...
                    if retries <= max_retries {
                        log::warn!(
                            "Master playlist fetch failed, retrying ({}/{}): {}",
                            retries,
                            max_retries,
                            last_error
                        );
        ...
        Err(format!(
            "Master playlist not available after {} retries: {}",
            max_retries, last_error
        ))
    }
                ...
                if i + 1 < lines.len() {
                    ...
                    if !playlist_path.starts_with('#') {
                        ...
                        let playlist_url = if playlist_path.starts_with("http")
                        {
                            ...
                            format!(
                                "{}/transcode/{}/{}",
                                self.server_url, media_id, playlist_path
                            )
                        };
    ...
    pub async fn fetch_variant_playlist(
        &self,
        variant: &Variant,
    ...
    pub async fn fetch_segment(
        ...
        // Cache the segment and limit cache size
        {
            ...
            inner
                .segment_cache
                .insert(segment.url.clone(), data.clone());
    ...
    fn record_bandwidth(&self, bandwidth: u64) {
        ...
        inner
            .bandwidth_history
            .retain(|(time, _)| now.duration_since(*time).as_secs() < 30);
    }
    ...
    pub fn should_switch_variant<'a>(
        &self,
        master_playlist: &'a MasterPlaylist,
    ) -> Option<&'a Variant> {
        let avg_bandwidth = self.calculate_average_bandwidth();
        ...
        if avg_bandwidth > current.bandwidth * 120 / 100 {
            // 20% headroom
...
/transcode/123/variant/720p/playlist.m3u8"#;
        ...
        assert_eq!(playlist.variants[0].profile, "transcode");
    }
