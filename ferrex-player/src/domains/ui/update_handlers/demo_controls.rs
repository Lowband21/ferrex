...
pub fn handle_movies_input(state: &mut State, value: String) -> DomainUpdateResult {
    state.domains.library.state.demo_controls.movies_input = value;
    state.domains.library.state.demo_controls.error = None;
    DomainUpdateResult::task(Task::none())
}

pub fn handle_series_input(state: &mut State, value: String) -> DomainUpdateResult {
...
pub fn handle_apply_sizing(state: &mut State) -> DomainUpdateResult {
    let controls = &mut state.domains.library.state.demo_controls;
    match build_demo_reset_request(&controls.movies_input, &controls.series_input) {
        Ok(request) => {
            controls.error = None;
...
fn parse_demo_numeric(input: &str) -> Result<Option<usize>, &'static str> {
    ...
    let value: usize = trimmed.parse().map_err(|_| "must be a positive integer")?;
...
fn build_demo_reset_request(
    ...
    let movie_count = parse_demo_numeric(movies_input).map_err(|err| format!("Movies {err}"))?;
    let series_count = parse_demo_numeric(series_input).map_err(|err| format!("Series {err}"))?;
