...
pub fn view_all_content<'a>(state: &'a State) -> Element<'a, Message> {
    ...
    if let Some(tab) = state.tab_manager.get_tab(TabId::All) {
        if let TabState::All(all_state) = tab {
            ...
            if !all_state.continue_watching.is_empty() {
                    ...
                    state.domains.ui.state.carousel_registry.get(&key)
                {
                    ...
                    let carousel = virtual_carousel::virtual_carousel(
                        ...
                        move |idx| {
                            ids_for_closure.get(idx).and_then(|uuid| {
                                ...
                                if acc
                                    .get(&MediaID::Movie(MovieID(*uuid)))
                                    .is_ok()
                                {
                                    return Some(
                        ...
                        state.domains.ui.state.carousel_registry.get(&key)
                    {
                        ...
                        let ctx =
                            vchelper::build_mixed_poster_context(state, &all);
                        if let Some(handle) =
                    ...
                    state.tab_manager.get_tab(TabId::Library(*library_id))
                {
                    ...
                    ) = tab
                    {
                            ...
                            state.domains.ui.state.carousel_registry.get(&key)
                        {
                            let cf = &state.domains.ui.state.carousel_focus;
                            let now = Instant::now();
                            let hover_preferred = cf.should_prefer_hover(
                                now,
                                HOVER_SWITCH_WINDOW_MS,
                            );
                            let is_active = (hover_preferred
                                && cf.hovered_key.as_ref() == Some(&key))
                                || (cf.keyboard_active_key.as_ref()
                                    == Some(&key));
    ...
    let scroll_id = if let Some(tab) = state.tab_manager.get_tab(TabId::All) {
        if let TabState::All(all_state) = tab {
