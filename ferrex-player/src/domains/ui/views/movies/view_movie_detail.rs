...
pub fn view_movie_detail<'a>(state: &'a State, movie_id: MovieID) -> Element<'a, Message> {
    ...
    match state.domains.ui.state.movie_yoke_cache.peek(&movie_uuid) { Some(yoke_arc) => {
        ...
        let movie_details_opt =
            if let MediaDetailsOption::Details(TmdbDetails::Movie(movie_details)) = movie_details {
                log::debug!(
        ...
        if let ArchivedMediaDetailsOption::Details(ArchivedTmdbDetails::Movie(ref movie_details)) =
            movie.details
        {
