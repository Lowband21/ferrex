...
use super::{TabId, TabState, AllTabState, LibraryTabState};
...
impl TabManager {
    ...
    pub fn update_libraries(&mut self, libraries: &[Library]) {
        ...
        let valid_ids: Vec<TabId> = self.tabs.keys()
            .filter(|tab_id| {
                match tab_id {
                    TabId::All => true,
        ...
        if !self.tabs.contains_key(&tab_id) {
            match tab_id {
                TabId::All => {
                    ...
                    self.tabs.insert(tab_id, TabState::new_all(self.store.clone()));
                }
                TabId::Library(library_id) => {
                    ...
                    if let Some(&library_type) = self.library_info.get(&library_id) {
                        ...
                        log::warn!("Attempted to create tab for unregistered library: {}", library_id);
        ...
        match tab_id {
            ...
            TabId::Library(library_id) => {
                if self.library_info.contains_key(&library_id) {
                    ...
                    log::warn!("Attempted to activate unregistered library tab: {}", library_id);
                    false
                }
            }
        }
    ...
    pub fn set_active_tab_with_scroll(
        ...
        if success {
            // Restore the new tab's scroll position from ScrollPositionManager
            if let Some(scroll_state) = scroll_manager.get_tab_scroll(&tab_id) {
                // Apply the saved scroll position to the tab's grid state
