...
use super::service::{FetchPriority, MetadataFetchService};
...
impl MetadataCoordinator {
    ...
    pub fn queue_metadata_for_view(&self, state: &crate::state_refactored::State, library_id: Uuid) {
        ...
        log::info!(
            "Queuing metadata for current view in library {}",
            library_id
        );

        // Get metadata needs from all active view models
        // In the library-centric design, we fetch metadata for all visible content
        let metadata_needs = state.all_view_model.get_metadata_needs();
    ...
    pub fn queue_metadata_for_all_views(&self, state: &crate::state_refactored::State, library_id: Uuid) {
        let Some(service) = &self.metadata_service else {
            log::warn!("MetadataCoordinator: No metadata service available");
            return;
        };
        ...
        let mut all_items: std::collections::HashMap<
            (crate::infrastructure::api_types::MediaId, FetchPriority),
            Uuid,
        > = std::collections::HashMap::new();
    ...
    pub fn update_visibility_and_queue(&self, state: &mut crate::state_refactored::State, library_id: Uuid) {
        log::debug!(
            "Updating visibility and queuing metadata for current view"
        );

        // Update visibility in all view models
        // In the library-centric design, all view models update based on current library
