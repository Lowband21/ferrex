use crate::domains::metadata::messages::Message as MetadataMessage;
use crate::infrastructure::api_types::{
    ApiResponse, CreateLibraryRequest, Library, LibraryReference, LibraryType, UpdateLibraryRequest,
};
use crate::infrastructure::services::api::ApiService;
use chrono::Utc;
use serde::{Deserialize, Serialize};

// TODO: Remove this duplicate type and use ferrex_core::media::MediaFile everywhere
...
// Conversion from core MediaFile type
impl From<ferrex_core::media::MediaFile> for MediaFile {
    fn from(core_file: ferrex_core::media::MediaFile) -> Self {
        Self {
            id: core_file.id.to_string(),
            filename: core_file.filename,
            path: core_file.path.to_string_lossy().to_string(),
            size: core_file.size,
            created_at: core_file.created_at.to_rfc3339(),
            metadata: core_file.media_file_metadata.map(|m| MediaMetadata {
                duration: m.duration,
                width: m.width,
                height: m.height,
                ...
                color_primaries: m.color_primaries,
                color_transfer: m.color_transfer,
                color_space: m.color_space,
                bit_depth: m.bit_depth,
                parsed_info: None, // These fields don't exist in core
                external_info: None,
            }),
            library_id: Some(core_file.library_id.to_string()),
        }
    }
}
...
pub async fn fetch_movies(
    server_url: String,
) -> anyhow::Result<Vec<crate::domains::media::models::MovieDetails>> {
    ...
    if let Some(movies) = json.get("movies") {
        let movie_list: Vec<crate::domains::media::models::MovieDetails> =
            serde_json::from_value(movies.clone())?;
        log::info!(
            "Fetched {} movies in {:?}",
            movie_list.len(),
            start.elapsed()
...
pub async fn fetch_movie_details(
    server_url: String,
    movie_id: String,
) -> anyhow::Result<crate::domains::media::models::MovieDetails> {
    ...
    if let Some(movie) = json.get("movie") {
        let movie_details: crate::domains::media::models::MovieDetails =
            serde_json::from_value(movie.clone())?;
...
pub async fn fetch_tv_shows(
    server_url: String,
) -> anyhow::Result<Vec<crate::domains::media::models::TvShowDetails>> {
    let client = reqwest::Client::new();
    ...
    if let Some(shows) = json.get("shows") {
        let show_list: Vec<crate::domains::media::models::TvShowDetails> =
            serde_json::from_value(shows.clone())?;
...
pub async fn fetch_tv_show_details(
    server_url: String,
    show_name: String,
) -> anyhow::Result<crate::domains::media::models::TvShowDetails> {
    let client = reqwest::Client::new();
    let response = client
        .get(format!(
            "{}/shows/{}",
    ...
    if let Some(show) = json.get("show") {
        let show_details: crate::domains::media::models::TvShowDetails =
            serde_json::from_value(show.clone())?;
...
pub async fn fetch_season_details(
    ...
    show_name: String,
    season_num: u32,
) -> anyhow::Result<crate::domains::media::models::SeasonDetails> {
    let client = reqwest::Client::new();
    let response = client
        .get(format!(
            "{}/shows/{}/seasons/{}",
    ...
    if let Some(season) = json.get("season") {
        let season_details: crate::domains::media::models::SeasonDetails =
            serde_json::from_value(season.clone())?;
        log::info!(
        ...
        if let Some(library_refs) = api_response.data {
            ...
            let library_list: Vec<Library> = library_refs
                ...
                .map(|lib_ref| Library {
                    ...
                    paths: lib_ref.paths.iter().map(|p| PathBuf::from(p)).collect(),
        ...
        if let Some(library_id) = api_response.data {
            ...
            if let Some(library_ref) = library_response.data {
                ...
                let created_library = Library {
                    ...
                    paths: library_ref.paths.iter().map(|p| PathBuf::from(p)).collect(),
                    ...
                    max_retry_attempts: 3,
                    created_at: Utc::now(),
                    updated_at: Utc::now(), // Empty initially, will be populated when media is loaded
                };
                log::info!(
...
use crate::infrastructure::api_types::{
    LibraryMediaResponse, MediaId, MediaReference, SeasonReference,
};
...
pub async fn fetch_media_details_batch(
    api_service: &crate::infrastructure::adapters::api_client_adapter::ApiClientAdapter,
    library_id: Uuid,
    media_ids: Vec<MediaId>,
) -> anyhow::Result<crate::infrastructure::api_types::BatchMediaResponse> {
    if media_ids.is_empty() {
        return Ok(crate::infrastructure::api_types::BatchMediaResponse {
            items: vec![],
            errors: vec![],
        });
    }
    ...
    let request = crate::infrastructure::api_types::BatchMediaRequest {
        library_id,
        media_ids,
    };

    // Use the authenticated API service
    let response: Result<crate::infrastructure::api_types::BatchMediaResponse, anyhow::Error> =
        api_service.post("/api/media/batch", &request).await
            .map_err(|e| anyhow::anyhow!("Failed to fetch batch: {}", e));
...
pub async fn fetch_series_season_details(
    server_url: String,
    library_id: Uuid,
    _series_id: crate::infrastructure::api_types::SeriesID,
    ...
    let tasks: Vec<_> = season_refs
        ...
        .map(|season| {
            ...
            tokio::spawn(async move {
                ...
                {
                    Ok(media) => media.as_season().cloned().or_else(|| {
