...
pub fn subscription(state: &State) -> Subscription<DomainMessage> {
    ...
    if matches!(&state.domains.ui.state.view, crate::domains::ui::types::ViewState::Player)
        && state.domains.player.state.video_opt.is_some()
        && state.domains.player.state.controls
    {
        subs.push(
            iced::time::every(std::time::Duration::from_millis(500)).map(|_| {
                DomainMessage::Player(Message::CheckControlsVisibility)
            }),
        );
    }
...
fn keyboard_shortcuts() -> Subscription<DomainMessage> {
    iced::keyboard::on_key_press(|key, modifiers| {
        ...
        let msg = match key {
            ...
            Key::Character(c) if c.as_str() == "f" || c.as_str() == "F" => Some(Message::ToggleFullscreen),
            Key::Named(Named::F11) => Some(Message::ToggleFullscreen),
            Key::Character(c) if c.as_str() == "m" || c.as_str() == "M" => Some(Message::ToggleMute),
            Key::Character(c) if c.as_str() == "s" || c.as_str() == "S" => {
                if modifiers.shift() { Some(Message::ToggleSubtitleMenu) } else { Some(Message::CycleSubtitleSimple) }
            }
            Key::Character(c) if c.as_str() == "a" || c.as_str() == "A" => Some(Message::CycleAudioTrack),
            _ => None,
        };
