use crate::auth_dto::{UserDto, UserListItemDto};
use crate::auth_errors::{AuthError, AuthResult, DeviceError, NetworkError, StorageError, TokenError, ValidationError};
...
use crate::auth_storage::{AuthStorage, StoredAuth as StoredAuthData};
use crate::hardware_fingerprint::generate_hardware_fingerprint;
...
/// JWT Token expiry buffer - refresh tokens 5 minutes before they expire
...
impl DeviceIdentity {
    ...
    pub async fn save(&self) -> AuthResult<()> {
        ...
        let content = serde_json::to_string_pretty(self)
            .map_err(|e| AuthError::Internal(format!("Failed to serialize device identity: {}", e)))?;
    ...
    fn config_path() -> AuthResult<PathBuf> {
        let proj_dirs = ProjectDirs::from("", "ferrex", "media-player")
            .ok_or_else(|| AuthError::Storage(StorageError::InitFailed("Unable to determine config directory".to_string())))?;
        Ok(proj_dirs.config_dir().join("device.json"))
    }
}

/// Re-export StoredAuth from auth_storage for backwards compatibility
pub use crate::auth_storage::StoredAuth;
...
impl AuthManager {
    ...
    pub async fn load_from_keychain(&self) -> AuthResult<Option<StoredAuth>> {
        ...
        let hardware_fingerprint = generate_hardware_fingerprint().await
            .map_err(|e| AuthError::Storage(StorageError::InitFailed(format!("Failed to get hardware fingerprint: {}", e))))?;
    ...
    async fn save_to_keychain(&self, auth: &StoredAuth) -> AuthResult<()> {
        ...
        self.auth_storage
            ...
            .await
            .map_err(|e| AuthError::Storage(StorageError::WriteFailed(std::io::Error::new(
    ...
    pub async fn save_current_auth(&self) -> AuthResult<()> {
        ...
        let stored_auth = self.auth_state.with_state(|state| {
            match state {
                AuthState::Authenticated { user, token, permissions, server_url } => {
                    Some(StoredAuth {
                        token: token.clone(),
                        user: user.clone(),
    ...
    pub async fn login(
        ...
        // Call login endpoint
        let token: AuthToken = self.api_client.post("/api/auth/login", &request).await
        ...
        // Get user profile
        let user: User = self.api_client.get("/api/users/me").await
            .map_err(|e| AuthError::Network(NetworkError::RequestFailed(e.to_string())))?;

        // Get user permissions
        let permissions: UserPermissions = self.api_client.get("/api/users/me/permissions").await
            .map_err(|e| AuthError::Network(NetworkError::RequestFailed(e.to_string())))?;
    ...
    pub async fn register(
        ...
        let token: AuthToken = self.api_client.post("/api/auth/register", &request).await
            .map_err(|e| AuthError::Network(NetworkError::RequestFailed(e.to_string())))?;
    ...
    pub async fn logout(&self) -> AuthResult<()> {
        ...
        tokio::spawn(async move {
            ...
            let _ = tokio::time::timeout(
                ...
                api_client.post::<EmptyRequest, serde_json::Value>("/api/auth/logout", &EmptyRequest {})
            ).await;
        });
    ...
    async fn get_or_create_device_id(&self) -> AuthResult<Uuid> {
        ...
        let id = Uuid::new_v4();
        let fingerprint = generate_hardware_fingerprint().await
            .map_err(|e| AuthError::Storage(StorageError::InitFailed(format!("Failed to get hardware fingerprint: {}", e))))?;
    ...
    pub async fn get_current_user(&self) -> Option<User> {
        self.auth_state.with_state(|state| {
            match state {
    ...
    pub async fn is_auto_login_enabled(&self) -> bool {
        if let Some(user) = self.get_current_user().await {
            ...
            let device_auto_login = self.auth_storage
    ...
    pub async fn get_all_users(&self) -> AuthResult<Vec<UserListItemDto>> {
        let users: Vec<UserListItemDto> = self.api_client.get("/api/users/public").await
            .map_err(|e| AuthError::Network(NetworkError::RequestFailed(e.to_string())))?;
        Ok(users)
    }
    ...
    pub async fn check_setup_status(&self) -> AuthResult<bool> {
        ...
        let status: SetupStatus = self.api_client.get("/api/setup/status").await
            .map_err(|e| AuthError::Network(NetworkError::RequestFailed(e.to_string())))?;
    ...
    pub async fn authenticate_device(
        ...
        let (device_id, _) = self.get_or_create_device_id().await
            .map(|id| (id, ()))
        ...
        let device_info = DeviceInfo {
            ...
            hardware_id: Some(generate_hardware_fingerprint().await
                .map_err(|e| AuthError::Storage(StorageError::InitFailed(format!("Failed to get hardware fingerprint: {}", e))))?),
        };
        ...
        let result: ServerAuthResult = self.api_client
...
fn is_token_expired(token: &AuthToken) -> bool {
    ...
    match decode_header(&token.access_token) {
        Ok(_) => {
            ...
            match decode::<JwtClaims>(&token.access_token, &DecodingKey::from_secret(b"dummy"), &validation) {
                Ok(token_data) => {
