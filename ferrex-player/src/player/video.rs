...
pub fn load_video(state: &mut State) -> Task<Message> {
    ...
    let _using_hls = state.player.using_hls;
    let _transcoding_duration = state.player.transcoding_duration;
    let tone_mapping_config_final = state.player.tone_mapping_config.clone();
