use crate::{
    ...
    server::{start_library_scan, start_media_scan},
    state::State,
};
...
pub fn handle_scan_library(state: &mut State, library_id: Uuid) -> Task<Message> {
...
pub fn handle_scan_all_libraries(state: &mut State) -> Task<Message> {
    ...
    Task::perform(
        start_media_scan(server_url, false, true),
        |result| match result {
            Ok(scan_id) => Message::ScanStarted(Ok(scan_id)),
            Err(e) => Message::ScanStarted(Err(e.to_string())),
        },
...
pub fn handle_force_rescan(state: &mut State) -> Task<Message> {
    ...
    Task::perform(
        start_media_scan(server_url, true, true),
        |result| match result {
            Ok(scan_id) => Message::ScanStarted(Ok(scan_id)),
            Err(e) => Message::ScanStarted(Err(e.to_string())),
        },
