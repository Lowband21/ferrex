use crate::{
    api_types::MediaReference, messages::library::Message, state::State, view_models::ViewModel,
};
use iced::Task;
...
pub fn handle_media_updated(state: &mut State, reference: MediaReference) -> Task<Message> {
    ...
    if let Some(library_id) = &state.current_library_id {
        if let Some(library) = state.libraries.iter_mut().find(|l| &l.id == library_id) {
            ...
            for media in &mut library.media {
    ...
    for (_, cache) in state.library_media_cache.iter_mut() {
...
pub fn handle_media_deleted(state: &mut State, file_id: String) -> Task<Message> {
    ...
    let media_ids_to_remove = if let Ok(store) = state.media_store.read() {
    ...
    if let Some(library_id) = &state.current_library_id {
        if let Some(library) = state.libraries.iter_mut().find(|l| &l.id == library_id) {
            library.media.retain(|media| match media {
    ...
    match &state.view {
        crate::state::ViewState::MovieDetail { movie, .. } => {
            if movie.file.id.to_string() == file_id {
                state.view = crate::state::ViewState::Library;
            }
        }
        crate::state::ViewState::EpisodeDetail { episode_id, .. } => {
            ...
            if let Ok(store) = state.media_store.read() {
                if let Some(MediaReference::Episode(episode)) = store.get(
                    ...
                    if episode.file.id.to_string() == file_id {
                        ...
                        state.view = crate::state::ViewState::TvShowDetail {
