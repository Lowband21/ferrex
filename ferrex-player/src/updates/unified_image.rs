use crate::{image_types::ImageRequest, messages::metadata::Message, state::State};
...
pub fn handle_unified_image_loaded(
    ...
    state.image_service.mark_loaded(&request, handle.clone());

    // Check if this is a backdrop for the current detail view
    let should_refresh = match (&state.view, &request.media_id, &request.size) {
        (
        ...
        (
            crate::state::ViewState::TvShowDetail { series_id, .. },
        ...
        (
            crate::state::ViewState::SeasonDetail { season_id, .. },
...
pub fn handle_unified_image_load_failed(
    ...
    state.image_service.mark_failed(&request, error);
