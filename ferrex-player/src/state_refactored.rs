...
use crate::domains::ui::view_models::{AllViewModel, MoviesViewModel, TvViewModel};
...
pub struct State {
    ...
    /// View models - these aggregate data from multiple domains
...
impl State {
    ...
    pub fn new(server_url: String) -> Self {
        ...
        let (image_service, _receiver) =
            crate::domains::metadata::image_service::UnifiedImageService::new(4);
        ...
        let auth_state = crate::domains::auth::AuthDomainState::new(
            api_adapter.clone(),
            auth_service.clone(),
        );
        ...
        let settings_adapter = crate::infrastructure::services::settings::SettingsApiAdapter::new(api_arc);
        ...
        let api_arc = std::sync::Arc::new(api_client.clone());
        let streaming_adapter = crate::infrastructure::services::streaming::StreamingApiAdapter::new(api_arc);
        ...
        if service_builder.toggles().prefer_trait_services {
            let api_arc = std::sync::Arc::new(api_client.clone());
            let adapter = crate::infrastructure::services::user_management::UserAdminApiAdapter::new(api_arc);
        ...
        let media_store_notifier = Arc::new(crate::domains::media::store::MediaStoreNotifier::with_debounce(250));
        {
            if let Ok(mut store) = media_store.write() {
                store.subscribe(Arc::downgrade(&media_store_notifier) as Weak<dyn MediaStoreSubscriber>);
                log::info!("[MediaStore] Notifier subscribed for ViewModel refresh tracking");
            } else {
                log::error!("[MediaStore] Failed to subscribe notifier - ViewModels won't auto-refresh");
            }
        }
        
        // Create view models
        let all_view_model = AllViewModel::new(media_store.clone());
        let movies_view_model = MoviesViewModel::new(media_store.clone());
        let tv_view_model = TvViewModel::new(media_store.clone());
