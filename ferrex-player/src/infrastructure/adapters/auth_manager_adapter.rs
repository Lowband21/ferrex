...
impl AuthService for AuthManagerAdapter {
    ...
    async fn refresh_access_token(&self) -> RepositoryResult<()> {
        self.manager.refresh_access_token().await
    ...
    async fn enable_admin_pin_unlock(&self) -> RepositoryResult<()> {
        self.manager
            .enable_admin_pin_unlock()
            .await
            .map_err(|e| RepositoryError::QueryFailed(format!("Enable admin PIN unlock failed: {}", e)))
    }
    
    async fn disable_admin_pin_unlock(&self) -> RepositoryResult<()> {
        self.manager
            .disable_admin_pin_unlock()
            .await
            .map_err(|e| RepositoryError::QueryFailed(format!("Disable admin PIN unlock failed: {}", e)))
    }
    ...
    async fn authenticate_device(&self, username: String, password: String, remember_device: bool) -> RepositoryResult<crate::domains::auth::manager::PlayerAuthResult> {
        self.manager
            ...
            .await
            .map_err(|e| RepositoryError::QueryFailed(format!("Device authentication failed: {}", e)))
    }
    ...
    async fn authenticate(&self, user: User, token: AuthToken, permissions: UserPermissions, server_url: String) -> RepositoryResult<()> {
        self.manager.auth_state().authenticate(user, token, permissions, server_url);
        Ok(())
    }
