    ...
    fn verify(&self) -> Result<(), String> {
        if let Some(expected) = self.expected_calls {
            if self.call_count != expected {
                return Err(format!("Expected {} calls, got {}", expected, self.call_count));
            }
        }
        Ok(())
    }
...
impl MockRegistry {
    ...
    pub fn verify_all(&self) -> Result<(), Vec<String>> {
        let errors: Vec<String> = self.mocks
            .lock()
            .unwrap()
            .values()
...
macro_rules! mock_service {
    ($name:ident) => {
        pub struct $name {
            inner: $crate::infrastructure::testing::mocks::SimpleMock,
        }
        ...
        impl $crate::infrastructure::testing::mocks::MockService for $name {
            fn reset(&mut self) {
                self.inner.reset()
            }
...
mod tests {
    ...
    fn test_domain_mock() {
        ...
        let result = registry.with_mock_mut::<UserService, DomainMock, _, _>(|mock| {
    ...
    fn test_mock_reset() {
        ...
        let count = registry.with_mock::<UserService, SimpleMock, _, _>(|mock| {
            mock.call_count()
        });
