    ...
    S: Default + Send,
{
    ...
    pub fn build(self) -> GenericDomainContext<S> {
        use crate::infrastructure::testing::time::SystemTimeProvider;
        
        GenericDomainContext {
            state: self.state.unwrap_or_default(),
            events: Vec::new(),
            executor: self.executor.unwrap_or_default(),
            time_provider: self.time_provider.unwrap_or_else(|| Box::new(SystemTimeProvider)),
...
mod tests {
    ...
    fn test_context_builder() {
        ...
        let ctx = DomainContextBuilder::<TestState>::new()
            .with_state(TestState { name: "test".to_string() })
