...
pub trait ServiceDependency: Send + Sync {
    ...
    fn execute_command(&mut self, command: &str) -> Pin<Box<dyn Future<Output = Result<(), String>> + Send + '_>>;
    
    /// Execute a query on the service
    fn execute_query(&self, query: &str) -> Pin<Box<dyn Future<Output = Result<String, String>> + Send + '_>>;
}
...
impl MockService {
    ...
    pub fn set_query_response(&self, query: impl Into<String>, response: impl Into<String>) {
        self.query_responses.lock().unwrap().insert(query.into(), response.into());
    }
    
    /// Set an error for a command
    pub fn set_command_error(&self, command: impl Into<String>, error: impl Into<String>) {
        self.command_errors.lock().unwrap().insert(command.into(), error.into());
    }
...
impl ServiceDependency for MockService {
    fn execute_command(&mut self, command: &str) -> Pin<Box<dyn Future<Output = Result<(), String>> + Send>> {
    ...
    fn execute_query(&self, query: &str) -> Pin<Box<dyn Future<Output = Result<String, String>> + Send + '_>> {
        let response = self.query_responses
            .lock()
            .unwrap()
            .get(query)
            .cloned();
...
impl DomainBoundaryBuilder {
    ...
    pub fn build(self) -> DomainBoundary {
        let event_bus = self.event_bus.unwrap_or_else(|| Box::new(InMemoryEventBus::new()));
