...
impl TestRecorder {
    ...
    pub fn record_command_failure(&self, description: String, error: String) {
        self.record_operation(OperationType::Command, description, None, false, Some(error));
    }
    ...
    pub fn record_state_change(&self, description: String, new_state: String) {
        self.record_operation(OperationType::StateChange, description, Some(new_state), true, None);
    }
        ...
        for op in self.operations() {
            match op.op_type {
                OperationType::Command => {
                    replay.push_str(&format!("// [{:?}] Execute command: {}\n", op.timestamp, op.description));
                    if let Some(data) = &op.data {
                        replay.push_str(&format!("ctx.execute_command({});\n", data));
                    }
                }
                OperationType::Query => {
                    replay.push_str(&format!("// [{:?}] Execute query: {}\n", op.timestamp, op.description));
                    if let Some(data) = &op.data {
                ...
                OperationType::TimeAdvance => {
                    ...
                    replay.push_str(&format!("time_provider.advance(Duration::from_millis({}));\n", 
