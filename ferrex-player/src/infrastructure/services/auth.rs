...
pub trait AuthService: Send + Sync {
    ...
    async fn login(&self, username: String, pin: String, server_url: String) -> RepositoryResult<(User, UserPermissions)>;
    ...
    async fn get_all_users(&self) -> RepositoryResult<Vec<crate::domains::auth::dto::UserListItemDto>>;
    
    /// Check device authentication status for a user
    async fn check_device_auth(&self, user_id: Uuid) -> RepositoryResult<crate::domains::auth::manager::DeviceAuthStatus>;
    ...
    async fn authenticate_device(&self, username: String, password: String, remember_device: bool) -> RepositoryResult<crate::domains::auth::manager::PlayerAuthResult>;
    
    /// Authenticate with PIN
    async fn authenticate_pin(&self, user_id: Uuid, pin: String) -> RepositoryResult<crate::domains::auth::manager::PlayerAuthResult>;
    ...
    async fn authenticate(&self, user: User, token: AuthToken, permissions: UserPermissions, server_url: String) -> RepositoryResult<()>;
...
pub mod mock {
    ...
    impl AuthService for MockAuthService {
        async fn login(&self, username: String, pin: String, _server_url: String) -> RepositoryResult<(User, UserPermissions)> {
            ...
            if let Some(user) = self.users.read().await.values().next().cloned() {
                Ok((user.clone(), UserPermissions { user_id: user.id, roles: vec![], permissions: HashMap::new(), permission_details: None }))
            } else {
        ...
        async fn check_device_auth(&self, _user_id: Uuid) -> RepositoryResult<crate::domains::auth::manager::DeviceAuthStatus> {
        ...
        async fn authenticate_device(&self, _username: String, _password: String, _remember_device: bool) -> RepositoryResult<crate::domains::auth::manager::PlayerAuthResult> {
            let user = User {
                id: Uuid::new_v4(),
                username: "test".into(),
                display_name: "Test".into(),
        ...
        async fn authenticate_pin(&self, _user_id: Uuid, _pin: String) -> RepositoryResult<crate::domains::auth::manager::PlayerAuthResult> {
            let user = User {
                id: Uuid::new_v4(),
                username: "test".into(),
                display_name: "Test".into(),
        ...
        async fn authenticate(&self, user: User, _token: AuthToken, _permissions: UserPermissions, _server_url: String) -> RepositoryResult<()> {
            self.users.write().await.insert(user.id, user);
            Ok(())
        }
