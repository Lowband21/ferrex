...
use ferrex_core::media::{MovieReference, SeriesReference, EpisodeReference, SeasonReference};
use ferrex_core::media::{EnhancedMovieDetails, EnhancedSeriesDetails, SeasonDetails, EpisodeDetails};
...
pub trait MetadataService: Send + Sync {
    ...
    /// Get season details
    async fn get_season_details(&self, series_id: u64, season_number: u8) -> RepositoryResult<SeasonDetails>;
    
    /// Get episode details
    async fn get_episode_details(&self, series_id: u64, season_number: u8, episode_number: u8) -> RepositoryResult<EpisodeDetails>;
    
    /// Batch fetch metadata for multiple movies
    async fn batch_fetch_movies(&self, tmdb_ids: Vec<u64>) -> RepositoryResult<Vec<EnhancedMovieDetails>>;
    
    /// Batch fetch metadata for multiple series
    async fn batch_fetch_series(&self, tmdb_ids: Vec<u64>) -> RepositoryResult<Vec<EnhancedSeriesDetails>>;
...
pub mod mock {
    ...
    impl MetadataService for MockMetadataService {
        async fn search_movies(&self, query: &str) -> RepositoryResult<Vec<MetadataSearchResult>> {
            self.search_movies_called.write().await.push(query.to_string());
        ...
        async fn get_season_details(&self, _series_id: u64, season_number: u8) -> RepositoryResult<SeasonDetails> {
            Ok(SeasonDetails {
                id: 1,
                season_number,
        ...
        async fn get_episode_details(&self, _series_id: u64, season_number: u8, episode_number: u8) -> RepositoryResult<EpisodeDetails> {
            Ok(EpisodeDetails {
                id: 1,
                episode_number,
                season_number,
        ...
        async fn update_series_metadata(&self, _series: &mut SeriesReference) -> RepositoryResult<()> {
            // Mock implementation - just return success
            Ok(())
        }
