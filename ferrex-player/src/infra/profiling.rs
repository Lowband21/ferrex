...
fn start_puffin_server() {
    ...
    static PUFFIN_SERVER: OnceCell<Option<puffin_http::Server>> =
        OnceCell::new();

    let server_addr = std::env::var("PUFFIN_SERVER_ADDR")
        .unwrap_or_else(|_| "127.0.0.1:8585".to_string());

    PUFFIN_SERVER.get_or_init(|| {
        match puffin_http::Server::new(server_addr.as_str()) {
            Ok(server) => {
            ...
            Err(err) => {
                if let Some(io_err) = err.downcast_ref::<std::io::Error>() {
                    match io_err.kind() {
                        ErrorKind::AddrInUse => {
                            log::warn!(
                                "Puffin data server already running on {}",
                                server_addr
                            );
                        }
                        _ => {
                            log::error!(
                                "Failed to start Puffin data server on {}: {}",
                                server_addr,
                                err
                            );
                        }
