use crate::api_types::MovieReference;
...
pub fn view_movie_detail<'a>(state: &'a State, movie: &'a MovieReference) -> Element<'a, Message> {
    ...
    let movie_details_opt = if let crate::api_types::MediaDetailsOption::Details(
        crate::api_types::TmdbDetails::Movie(movie_details),
    ) = &movie.details
    {
        Some(movie_details)
    } else {
    ...
    if let Some(progress) = state.get_media_progress(&media_id) {
        if state.is_watched(&media_id) {
    ...
    // Button row
    let legacy_file = crate::api_types::movie_reference_to_legacy(movie, &state.server_url);
    ...
    if let MediaDetailsOption::Details(TmdbDetails::Movie(movie_details)) = &movie.details {
        let cast_section = crate::components::create_cast_scrollable(&movie_details.cast);
    ...
    let header_offset = crate::constants::layout::header::HEIGHT;
