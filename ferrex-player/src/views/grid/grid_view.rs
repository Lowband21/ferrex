...
fn calculate_grid_layout(window_width: f32) -> (usize, f32) {
    use crate::constants::{calculations, poster, scale_presets};
...
fn create_movie_card<'a>(
    ...
    let is_hovered = hovered_media_id
        ...
        .unwrap_or(false);
    crate::components::movie_reference_card_with_state(movie, is_hovered, is_visible, Some(state))
}
...
fn create_series_card<'a>(
    ...
    let is_hovered = hovered_media_id
        ...
        .unwrap_or(false);
    crate::components::series_reference_card_with_state(series, is_hovered, is_visible, Some(state))
}
