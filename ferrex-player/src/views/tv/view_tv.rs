use crate::{
    ...
    image_types::{ImageSize, Priority},
    messages::ui::Message,
    state::State,
    theme,
...
pub fn view_tv_show_detail<'a>(state: &'a State, _show_name: &'a str) -> Element<'a, Message> {
    ...
    let series_id = match &state.view {
        crate::state::ViewState::TvShowDetail { series_id, .. } => series_id.clone(),
        _ => {
    ...
    let series_ref = if let Ok(store) = state.media_store.read() {
        if let Some(MediaReference::Series(series)) = store.get(&MediaId::Series(series_id.clone()))
        {
            Some(series.clone())
        } else {
            None
    ...
    if let Some(series_ref) = series_ref {
        // Debug: Log what series we're displaying
        log::info!(
            "Displaying series: {} (ID: {})",
        ...
        // Stats - get season count from MediaStore
        let season_count = if let Ok(store) = state.media_store.read() {
            store.get_seasons(series_id.as_str()).len()
        } else {
            0
        };
        ...
        // Play button row - find first episode to play
        let first_episode = if let Ok(store) = state.media_store.read() {
            ...
            if let Some(season) = first_season {
                ...
                episodes.into_iter().next().cloned()
            } else {
                None
            }
        } else {
        ...
        if let Some(episode) = first_episode {
            let series_details = match &series_ref.details {
                crate::api_types::MediaDetailsOption::Details(
                    crate::api_types::TmdbDetails::Series(details),
                ) => Some(details),
                _ => None,
            };
            let legacy_file =
                crate::api_types::episode_reference_to_legacy(&episode, series_details);
            let button_row = crate::components::create_action_button_row(
        ...
        if season_count > 0 {
            ...
            if let Some(carousel_state) = &state.show_seasons_carousel {
                // Use seasons from state (already populated)
                let season_cards: Vec<_> = state
                    .current_show_seasons
                    .iter()
                    .map(|season| {
                        crate::components::season_reference_card_with_state(
                            season,
                            false,
                            Some(state),
                        )
                ...
                let seasons_carousel = crate::views::carousel::media_carousel(
    ...
    let aspect_button = crate::components::create_backdrop_aspect_button(state);
...
pub fn view_season_detail<'a>(
    ...
    let (series_ref, season_ref, episodes) = if let Ok(store) = state.media_store.read() {
        ...
        // Use episodes from state if available, otherwise fetch from store
        let episodes: Vec<_> = if !state.current_season_episodes.is_empty()
            && state
                .current_season_episodes
                .iter()
                .all(|ep| ep.season_id.as_str() == season_id.as_str())
        {
            state.current_season_episodes.clone()
        } else if let Some(ref season) = season_ref {
            store
    ...
    if let Some(season_ref) = season_ref {
        ...
        let air_dates: Vec<String> = episodes
            ...
            .filter_map(|e| match &e.details {
                crate::api_types::MediaDetailsOption::Details(
                    crate::api_types::TmdbDetails::Episode(details),
            ...
            if let Some(carousel_state) = &state.season_episodes_carousel {
                ...
                let episode_cards: Vec<_> = episodes
                    ...
                    .map(|(index, episode)| {
                        ...
                        let watch_progress =
                            state.get_media_progress(&MediaId::Episode(episode.id.clone()));
                ...
                let episodes_carousel = crate::views::carousel::media_carousel(
...
pub fn view_episode_detail<'a>(
    ...
    let (episode_ref, series_name) = if let Ok(store) = state.media_store.read() {
    ...
    if let Some(episode) = episode_ref {
        // Get watch progress for this episode
        let watch_progress = state.get_media_progress(&MediaId::Episode(episode_id.to_owned()));
        ...
        if let Some(progress) = watch_progress {
            if state.is_watched(&MediaId::Episode(episode_id.to_owned())) {
        ...
        let series_details = if let Ok(store) = state.media_store.read() {
        ...
        let legacy_file =
            crate::api_types::episode_reference_to_legacy(&episode, series_details.as_ref());
        let button_row = crate::components::create_action_button_row(
            Message::PlayMediaWithId(
                legacy_file,
                ferrex_core::api_types::MediaId::Episode(episode.id.clone()),
