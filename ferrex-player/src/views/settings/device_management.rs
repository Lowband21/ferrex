...
pub fn view_device_management<'a>(state: &'a State) -> Element<'a, Message> {
    let device_state = &state.device_management_state;
...
fn create_loading_view<'a>() -> Element<'a, Message> {
    container(
        column![
            row![
                icon_text(Icon::Loader).size(24).color(theme::MediaServerTheme::TEXT_SECONDARY),
                Space::with_width(10),
...
fn create_error_view_owned<'a>(error: String) -> Element<'a, Message> {
    container(
        column![
            row![
                icon_text(Icon::X).size(24).color(theme::MediaServerTheme::ERROR),
                Space::with_width(10),
...
fn create_empty_view<'a>() -> Element<'a, Message> {
    container(
        column![
            icon_text(Icon::Smartphone).size(48).color(theme::MediaServerTheme::TEXT_SUBDUED),
            Space::with_height(15),
                    ...
                    row![
                        ...
                        if device.is_current_device {
                            ...
                            .style(|theme: &Theme| {
                                ...
                                container::Style {
                                    background: Some(theme::MediaServerTheme::SUCCESS.scale_alpha(0.2).into()),
                                    border: Border {
                                        color: theme::MediaServerTheme::SUCCESS,
                                        width: 1.0,
                                        radius: 4.0.into(),
