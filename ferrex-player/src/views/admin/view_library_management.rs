...
use crate::{
    api_types::LibraryType, messages::ui::Message, permissions::StatePermissionExt, prelude::*,
    state::State, theme, views::admin::view_library_form,
};
use iced::{
...
pub fn view_library_management(state: &State) -> Element<Message> {
    ...
    if let Some(form_data) = &state.library_form_data {
        return view_library_form(state, form_data).map(|msg| match msg {
            crate::messages::library::Message::HideLibraryForm => Message::HideLibraryForm,
            crate::messages::library::Message::UpdateLibraryFormName(name) => {
                Message::UpdateLibraryFormName(name)
            }
            crate::messages::library::Message::UpdateLibraryFormType(t) => {
                Message::UpdateLibraryFormType(t)
            }
            crate::messages::library::Message::UpdateLibraryFormPaths(paths) => {
                Message::UpdateLibraryFormPaths(paths)
            }
            crate::messages::library::Message::UpdateLibraryFormScanInterval(interval) => {
                Message::UpdateLibraryFormScanInterval(interval)
            }
            crate::messages::library::Message::ToggleLibraryFormEnabled => {
                Message::ToggleLibraryFormEnabled
            }
            crate::messages::library::Message::SubmitLibraryForm => Message::SubmitLibraryForm,
...
fn create_library_card<'a>(
    library: &'a crate::media_library::Library,
    permissions: &crate::permissions::PermissionChecker,
) -> Element<'a, Message> {
        ...
        row![
            ...
            row![
                ...
                column![
                    ...
                    .align_y(iced::Alignment::Center),
                    text(library.paths.first().map_or("No paths", |s| s.as_str()))
                        .size(14)
                        .color(theme::MediaServerTheme::TEXT_SECONDARY),
                ]
                .spacing(5),
