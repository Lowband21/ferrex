...
impl AuthStorage {
    ...
    /// Derive key from device fingerprint for legacy v1 format (SHA256)
    /// This is only used for migration purposes
    fn derive_key_legacy(device_fingerprint: &str) -> Key<Aes256Gcm> {
        use sha2::{Digest, Sha256};
        
        let mut hasher = Sha256::new();
        hasher.update(device_fingerprint.as_bytes());
        hasher.update(b"ferrex-auth-v1"); // Use the old salt
    ...
    pub async fn load_auth(&self, device_fingerprint: &str) -> Result<Option<StoredAuth>> {
        ...
        let (key, needs_migration) = match encrypted_data.version {
            1 => {
                ...
                (Self::derive_key_legacy(device_fingerprint), true)
            }
            2 => {
                ...
                match encrypted_data.salt.as_ref() {
                    Some(salt_str) => {
                        ...
                        (Self::derive_key(device_fingerprint, salt.as_str().as_bytes())?, false)
                    }
                    None => {
    ...
    pub async fn is_auto_login_enabled(&self, user_id: &Uuid) -> Result<bool> {
        ...
        let auto_login_path = self.cache_path
            .parent()
    ...
    pub async fn is_admin_pin_unlock_enabled(&self) -> Result<bool> {
        let admin_unlock_path = self.cache_path
            .parent()
            .ok_or_else(|| anyhow::anyhow!("Invalid cache path"))?
