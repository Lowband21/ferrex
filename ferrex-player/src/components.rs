...
use crate::{media_card, state::State, theme};
...
/// Create a movie card for MovieReference with lazy metadata loading
pub fn movie_reference_card<'a>(
    movie: &'a crate::api_types::MovieReference,
    is_hovered: bool,
) -> Element<'a, Message> {
    use crate::api_types::{MediaDetailsOption, TmdbDetails};
    ...
    movie_reference_card_with_visibility(movie, is_hovered, false)
}

/// Create a movie card with visibility information for proper prioritization
pub fn movie_reference_card_with_visibility<'a>(
    movie: &'a crate::api_types::MovieReference,
    is_hovered: bool,
    is_visible: bool,
) -> Element<'a, Message> {
    movie_reference_card_with_state(movie, is_hovered, is_visible, None)
}
...
pub fn movie_reference_card_with_state<'a>(
    ...
    let priority = if is_hovered || is_visible {
        crate::image_types::Priority::Visible
    } else {
        crate::image_types::Priority::Preload
    };

    // Get watch progress from state if available
    let watch_progress = state.and_then(|s| {
        s.get_media_progress(&ferrex_core::api_types::MediaId::Movie(movie.id.clone()))
    });

    // Create the image element with progress indicator
    use crate::widgets::image_for;
    ...
    let mut img = image_for(media_id.clone())
        .size(crate::image_types::ImageSize::Poster)
        .rounded(8.0)
        .width(Length::Fixed(200.0))
        .height(Length::Fixed(300.0))
        .animation(crate::widgets::AnimationType::enhanced_flip())
        .placeholder(lucide_icons::Icon::Film)
        .priority(priority)
        .is_hovered(is_hovered)
        .on_play(Message::PlayMediaWithId(
            crate::api_types::to_legacy_media_file(&movie.file),
    ...
    if let Some(theme_color_str) = &movie.theme_color {
        if let Ok(color) = crate::views::macros::parse_hex_color(theme_color_str) {
...
/// Create a series card for SeriesReference with lazy metadata loading
pub fn series_reference_card<'a>(
    series: &'a crate::api_types::SeriesReference,
    is_hovered: bool,
) -> Element<'a, Message> {
    series_reference_card_with_visibility(series, is_hovered, false)
}

/// Create a series card with visibility information for proper prioritization
pub fn series_reference_card_with_visibility<'a>(
    series: &'a crate::api_types::SeriesReference,
    is_hovered: bool,
    is_visible: bool,
) -> Element<'a, Message> {
    series_reference_card_with_state(series, is_hovered, is_visible, None)
}
...
pub fn series_reference_card_with_state<'a>(
    ...
    let badge_element: Option<Element<'a, Message>> = if episode_count > 0 {
        Some(crate::widgets::episode_count_badge(episode_count).into())
    } else {
        None
    };
...
/// Create a season card for SeasonReference with lazy metadata loading
pub fn season_reference_card<'a>(
    season: &'a crate::api_types::SeasonReference,
    is_hovered: bool,
) -> Element<'a, Message> {
    season_reference_card_with_state(season, is_hovered, None)
}
...
pub fn season_reference_card_with_state<'a>(
    ...
    let episode_count = state
        .map(|s| s.get_season_episode_count(season_id))
        .unwrap_or(0);
...
/// Create an episode card for EpisodeReference with lazy metadata loading
pub fn episode_reference_card<'a>(
    episode: &'a crate::api_types::EpisodeReference,
    is_hovered: bool,
) -> Element<'a, Message> {
    episode_reference_card_with_state(episode, is_hovered, None)
}
...
pub fn episode_reference_card_with_state<'a>(
    ...
    use crate::api_types::{MediaDetailsOption, TmdbDetails};
    use crate::image_types::{ImageSize, Priority};
    use crate::views::macros::truncate_text;
    use crate::widgets::image_for;
    use iced::{
    ...
    let watch_progress = state.and_then(|s| {
        s.get_media_progress(&ferrex_core::api_types::MediaId::Episode(
            episode.id.clone(),
        ))
    });
    ...
    let poster_button = button(image_element)
        .on_press(Message::PlayMediaWithId(
            crate::api_types::to_legacy_media_file(&episode.file),
...
fn create_cast_card<'a>(actor: &'a CastMember) -> Element<'a, Message> {
    ...
    let profile_image =
        crate::widgets::image_for(ferrex_core::api_types::MediaId::Person(person_id))
            .size(crate::image_types::ImageSize::Profile)
            .width(Length::Fixed(card_width))
...
pub fn create_backdrop_aspect_button<'a>(state: &'a State) -> Element<'a, Message> {
    let aspect_button_text = match state.background_shader_state.backdrop_aspect_mode {
        crate::state::BackdropAspectMode::Auto => "Auto",
        crate::state::BackdropAspectMode::Force21x9 => "21:9",
    };
...
pub fn create_technical_details<'a>(
    metadata: &'a crate::api_types::MediaFileMetadata,
) -> Element<'a, Message> {
