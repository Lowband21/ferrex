...
async fn lockout_expires_after_timeout() {
    ...
    assert!(
        helper.auth_service.is_account_locked(user_id).await,
    ...
    helper
        .auth_service
        .advance_time(chrono::Duration::minutes(15))
        .await;

    // Account should be unlocked
    assert!(
        !helper.auth_service.is_account_locked(user_id).await,
        "Account should be unlocked after timeout"
    );
    ...
    assert!(
        result.is_ok(),
        "Authentication should succeed after lockout expires"
    );
}
...
async fn successful_login_resets_failed_attempts() {
    ...
    for i in 1..=4 {
        ...
        assert!(
            !is_locked,
            "Account should not be locked after {} attempts after reset",
            i
        );
    }
    ...
    assert!(
        helper.auth_service.is_account_locked(user_id).await,
        "Account should lock after 5 new attempts"
    );
}
