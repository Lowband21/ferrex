...
async fn first_user_becomes_admin_automatically() {
    ...
    assert!(auth.is_first_run().await, "System should be in first-run state");
    ...
    assert!(!permissions.roles.is_empty(), "First user should have roles assigned");
    assert!(
        permissions.roles.iter().any(|role| role.name == "admin"),
        "First user should have admin role, but got roles: {:?}",
        permissions.roles.iter().map(|r| &r.name).collect::<Vec<_>>()
    );
    ...
    assert!(!auth.is_first_run().await, "System should exit first-run after admin creation");
}
...
async fn cannot_create_user_with_duplicate_username() {
    ...
    assert!(result.is_err(), "Should not be able to create duplicate username");
