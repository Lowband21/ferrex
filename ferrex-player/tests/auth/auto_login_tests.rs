...
async fn auto_login_is_device_specific() {
    ...
    helper.auth_service.logout(session1).await.expect("Logout should succeed");
...
async fn manual_logout_disables_auto_login() {
    ...
    let session = helper.auth_service
    ...
    assert!(result.is_err(), "Auto-login should be disabled after explicit manual logout");
}
...
async fn another_user_login_disables_auto_login() {
    ...
    assert!(helper.auth_service.attempt_auto_login(device.clone()).await.is_ok(),
            "Auto-login should still work after normal logout");
    
    // User2 logs in on same device
    let _session2 = helper.auth_service
    ...
    let is_enabled = helper.auth_service
        .is_auto_login_enabled(user1_id, device.clone())
        .await;
    
    assert!(!is_enabled, "User1's auto-login should be disabled after user2 logs in");
}