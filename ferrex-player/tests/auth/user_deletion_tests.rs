...
async fn user_deletion_invalidates_all_sessions() {
    ...
    let admin_id = helper.auth_service
    ...
    let admin_session = helper.auth_service
    ...
    assert!(!helper.auth_service.is_session_valid(&session1).await, "Session1 should be invalid after user deletion");
    assert!(!helper.auth_service.is_session_valid(&session2).await, "Session2 should be invalid after user deletion");
...
async fn user_deletion_removes_trusted_devices() {
    ...
    helper.auth_service.trust_device(user_id, device1.clone()).await.unwrap();
    helper.auth_service.trust_device(user_id, device2.clone()).await.unwrap();
    
    // Verify devices are trusted
    assert!(helper.auth_service.is_device_trusted(user_id, &device1).await);
    assert!(helper.auth_service.is_device_trusted(user_id, &device2).await);
...
async fn only_admin_can_delete_users() {
    ...
    assert!(result.is_err(), "Non-admin should not be able to delete users");
