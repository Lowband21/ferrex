...
async fn device_trust_expires_after_30_days() {
    ...
    // Trust the device
    auth
        .trust_device(user_id, device_id.clone())
        .await
...
async fn admin_requires_password_on_untrusted_device() {
    ...
    auth
        .setup_pin(admin_id, "1234".to_string(), Some(admin_session.clone()))
        .await
    ...
    auth
        .trust_device(admin_id, device1.clone())
        .await
        .expect("Device trust should succeed");
    ...
    let result = auth
        ...
        .await;
    assert!(result.is_ok(), "Admin should be able to use PIN on trusted device");
    ...
    assert!(result.is_err(), "Admin should NOT be able to use PIN on untrusted device");
    ...
    let result = auth
        ...
        .await;
    assert!(result.is_ok(), "Admin should be able to use password on any device");
}
...
async fn device_can_be_revoked() {
    ...
    auth
        .revoke_device(device_id.clone(), admin_session)
        .await
