...
mod auth_basic {
    ...
    async fn test_device_trust_and_pin_auth() {
        ...
        assert!(result.is_err(), "Admin should NOT authenticate with PIN on untrusted device");
...
mod completed_tests {
    ...
    async fn auto_login_device_specific() {
        ...
        // Create a user
        let user_id = auth.create_user("testuser".to_string(), "password123".to_string())
            .await
        ...
        let _session = auth.authenticate_with_device(user_id, "password123".to_string(), device1.clone())
            .await
    ...
    async fn user_deletion_cascades_sessions() {
        ...
        let admin_id = auth.create_user("admin".to_string(), "admin_pass".to_string())
            .await
            .expect("Admin creation should succeed");
        
        // Create regular user
        let user_id = auth.create_user("user".to_string(), "user_pass".to_string())
            .await
            .expect("User creation should succeed");
        
        // Create sessions for the user
        let session1 = auth.authenticate_with_device(user_id, "user_pass".to_string(), "device1".to_string())
            .await
            .expect("Auth should succeed");
        
        let session2 = auth.authenticate_with_device(user_id, "user_pass".to_string(), "device2".to_string())
            .await
            .expect("Auth should succeed");
        
        // Trust a device
        auth.trust_device(user_id, "device1".to_string()).await
            .expect("Trust device should succeed");
        
        // Enable auto-login
        auth.enable_auto_login(user_id, "device1".to_string()).await
            .expect("Enable auto-login should succeed");
        
        // Verify everything is set up
        assert!(auth.is_session_valid(&session1).await);
        assert!(auth.is_session_valid(&session2).await);
        assert!(auth.is_device_trusted(user_id, &"device1".to_string()).await);
        assert!(auth.is_auto_login_enabled(user_id, "device1".to_string()).await);
        
        // Admin deletes the user
        let admin_session = auth.authenticate(admin_id, "admin_pass".to_string())
            .await
        ...
        assert!(!auth.is_session_valid(&session1).await, "Session1 should be invalid");
        assert!(!auth.is_session_valid(&session2).await, "Session2 should be invalid");
        assert!(!auth.is_device_trusted(user_id, &"device1".to_string()).await, "Device should not be trusted");
        assert!(!auth.is_auto_login_enabled(user_id, "device1".to_string()).await, "Auto-login should be disabled");
        assert!(auth.get_user(user_id).await.is_none(), "User should not exist");
    }
}