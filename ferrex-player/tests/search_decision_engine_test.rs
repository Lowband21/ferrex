...
mod search_decision_engine_tests {
    ...
    use ferrex_player::domains::search::metrics::{SearchPerformanceMetrics, PerformanceHistory, NetworkMonitor};
    ...
    fn test_complex_query_detection() {
        // Test simple query
        assert!(!SearchDecisionEngine::is_complex_query("simple", &[SearchField::Title]));
        ...
        assert!(SearchDecisionEngine::is_complex_query("test AND other", &[SearchField::Title]));
        assert!(SearchDecisionEngine::is_complex_query("test OR other", &[SearchField::Title]));
        assert!(SearchDecisionEngine::is_complex_query("\"exact match\"", &[SearchField::Title]));
        
        // Test complex query with cast/crew fields
        assert!(SearchDecisionEngine::is_complex_query("test", &[SearchField::Cast]));
        assert!(SearchDecisionEngine::is_complex_query("test", &[SearchField::Crew]));
    }
}