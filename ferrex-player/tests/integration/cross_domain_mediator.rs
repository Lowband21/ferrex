...
async fn test_user_logout_clears_authentication() {
    ...
    assert!(!state.is_authenticated, "Global authentication flag should be cleared");
    assert!(state.domains.auth.state.user_permissions.is_none(), "User permissions should be cleared");
}
...
async fn test_auth_command_routing() {
    ...
    assert_eq!(state.domains.auth.state.is_authenticated, initial_state,
...
async fn test_library_selection_updates_state() {
    ...
    assert_eq!(state.domains.library.state.current_library_id, None,
...
async fn test_library_select_all_clears_selection() {
    ...
    state.domains.library.state.current_library_id = Some(Uuid::new_v4());
...
async fn test_mediator_processes_multiple_events() {
    ...
    for event in update_result.events {
        tasks.push(ferrex_player::common::messages::cross_domain::handle_event(&mut state, event));
    }
...
mod event_ordering_tests {
    ...
    async fn test_event_order_preserved() {
        ...
        let events = vec![
            CrossDomainEvent::LibrarySelected(Uuid::new_v4()),
            CrossDomainEvent::LibrarySelectAll,
            CrossDomainEvent::LibrarySelected(Uuid::new_v4()),
        ];
        ...
        assert!(state.domains.library.state.current_library_id.is_some(),
