use ferrex_player::{
    common::messages::{DomainMessage, CrossDomainEvent, DomainUpdateResult},
...
async fn test_libraries_loaded_after_authentication() {
    ...
    assert!(!auth_result.events.is_empty(), "Auth domain should emit events after LoginSuccess");
    ...
    assert!(has_auth_complete, "AuthenticationComplete event should be emitted");
    assert!(has_user_authenticated, "UserAuthenticated event should be emitted");
    ...
    assert!(has_auth_complete, "Authentication should trigger AuthenticationComplete event");
    assert!(has_user_authenticated, "Authentication should trigger UserAuthenticated event");
    
    // Verify state is updated correctly
    assert!(state.is_authenticated, "State should be marked as authenticated");
    assert!(state.domains.auth.state.user_permissions.is_some(), "User permissions should be stored");
...
async fn test_authentication_complete_triggers_load_libraries() {
    ...
    assert!(true, "Cross-domain handler should process AuthenticationComplete");
...
async fn test_complete_login_to_library_flow() {
    ...
    assert_eq!(
        state.domains.auth.state.user_permissions.as_ref().unwrap().user_id,
        user.id,
