...
pub fn build_cast_member<'a>(
    ...
    let original_name =
        c.original_name.as_deref().map(|s| builder.create_string(s));
    let character = builder.create_string(&c.character);
    let profile_path =
        c.profile_path.as_deref().map(|s| builder.create_string(s));
...
pub fn build_person_external_ids<'a>(
    ...
    fb::PersonExternalIds::create(
        builder,
        &fb::PersonExternalIdsArgs {
            imdb_id: imdb,
...
pub fn build_external_ids<'a>(
    ...
    let freebase_mid = ids
        .freebase_mid
        .as_deref()
        .map(|s| builder.create_string(s));

    fb::ExternalIds::create(
        builder,
        &fb::ExternalIdsArgs {
            imdb_id: imdb,
...
pub fn build_spoken_language<'a>(
    ...
    fb::SpokenLanguage::create(
        builder,
        &fb::SpokenLanguageArgs {
            iso_639_1: iso,
            name: Some(name),
        },
    )
...
pub fn build_production_company<'a>(
    ...
    let country = p
        ...
        .as_deref()
        .map(|s| builder.create_string(s));
    fb::ProductionCompany::create(
        builder,
        &fb::ProductionCompanyArgs {
            id: p.id,
            name: Some(name),
...
pub fn build_production_country<'a>(
    ...
    let name = builder.create_string(&p.name);
    fb::ProductionCountry::create(
        builder,
        &fb::ProductionCountryArgs {
            iso_3166_1: Some(iso),
            name: Some(name),
        },
    )
...
pub fn build_media_images<'a>(
    ...
    let posters: Vec<_> = imgs
        .posters
        .iter()
        .map(|i| build_image_with_metadata(builder, i))
        .collect();
    let backdrops: Vec<_> = imgs
        .backdrops
        .iter()
        .map(|i| build_image_with_metadata(builder, i))
        .collect();
    let logos: Vec<_> = imgs
        .logos
        .iter()
        .map(|i| build_image_with_metadata(builder, i))
        .collect();
    let stills: Vec<_> = imgs
        .stills
        .iter()
        .map(|i| build_image_with_metadata(builder, i))
        .collect();

    let posters = if posters.is_empty() {
        None
    } else {
        Some(builder.create_vector(&posters))
    };
    let backdrops = if backdrops.is_empty() {
        None
    } else {
        Some(builder.create_vector(&backdrops))
    };
    let logos = if logos.is_empty() {
        None
    } else {
        Some(builder.create_vector(&logos))
    };
    let stills = if stills.is_empty() {
        None
    } else {
        Some(builder.create_vector(&stills))
    };

    fb::MediaImages::create(
        builder,
        &fb::MediaImagesArgs {
            posters,
            backdrops,
...
fn build_image_with_metadata<'a>(
    ...
    fb::ImageWithMetadata::create(
        builder,
        &fb::ImageWithMetadataArgs {
...
pub fn build_enhanced_movie_details<'a>(
    ...
    let release_date =
        d.release_date.as_deref().map(|s| builder.create_string(s));
    let content_rating = d
        .content_rating
        .as_deref()
        .map(|s| builder.create_string(s));
    ...
    let poster_path =
        d.poster_path.as_deref().map(|s| builder.create_string(s));
    let backdrop_path =
        d.backdrop_path.as_deref().map(|s| builder.create_string(s));
    let logo_path = d.logo_path.as_deref().map(|s| builder.create_string(s));

    // Image IDs
    let primary_poster_iid =
        d.primary_poster_iid.as_ref().map(|id| uuid_to_fb(id));
    let primary_backdrop_iid =
        d.primary_backdrop_iid.as_ref().map(|id| uuid_to_fb(id));

    // Vectors
    let genres: Vec<_> =
        d.genres.iter().map(|g| build_genre(builder, g)).collect();
    let genres = if genres.is_empty() {
        None
    } else {
        Some(builder.create_vector(&genres))
    };

    let spoken_languages: Vec<_> = d
        .spoken_languages
        .iter()
        .map(|l| build_spoken_language(builder, l))
        .collect();
    let spoken_languages = if spoken_languages.is_empty() {
        None
    } else {
        Some(builder.create_vector(&spoken_languages))
    };

    let production_companies: Vec<_> = d
        .production_companies
        .iter()
        .map(|p| build_production_company(builder, p))
        .collect();
    let production_companies = if production_companies.is_empty() {
        None
    } else {
        Some(builder.create_vector(&production_companies))
    };

    let production_countries: Vec<_> = d
        .production_countries
        .iter()
        .map(|p| build_production_country(builder, p))
        .collect();
    let production_countries = if production_countries.is_empty() {
        None
    } else {
        Some(builder.create_vector(&production_countries))
    };

    let cast: Vec<_> = d
        .cast
        .iter()
        .map(|c| build_cast_member(builder, c))
        .collect();
    let cast = if cast.is_empty() {
        None
    } else {
        Some(builder.create_vector(&cast))
    };
...
pub fn build_enhanced_series_details<'a>(
    ...
    let original_name =
        d.original_name.as_deref().map(|s| builder.create_string(s));
    let overview = d.overview.as_deref().map(|s| builder.create_string(s));
    let first_air_date = d
        .first_air_date
        .as_deref()
        .map(|s| builder.create_string(s));
    let last_air_date =
        d.last_air_date.as_deref().map(|s| builder.create_string(s));
    let content_rating = d
        .content_rating
        .as_deref()
        .map(|s| builder.create_string(s));
...
pub fn build_episode_details<'a>(
    ...
    let primary_still_iid =
        d.primary_still_iid.as_ref().map(|id| uuid_to_fb(id));
    let production_code = d
        .production_code
        .as_deref()
        .map(|s| builder.create_string(s));
    let external_ids = build_external_ids(builder, &d.external_ids);
    ...
    let guest_stars: Vec<_> = d
        .guest_stars
        .iter()
        .map(|c| build_cast_member(builder, c))
        .collect();
    let guest_stars = if guest_stars.is_empty() {
        None
    } else {
        Some(builder.create_vector(&guest_stars))
    };
