...
use crate::conversions::common::{library_type_to_fb, timestamp_to_fb, option_timestamp_to_fb};
...
pub fn build_library<'a>(
    ...
    fb::Library::create(builder, &fb::LibraryArgs {
        id: Some(&id),
        name: Some(name),
...
pub fn serialize_library_reference_list(
    ...
    let mut builder = FlatBufferBuilder::with_capacity(512 * libraries.len().max(1));
    ...
    let list = fb::LibraryList::create(&mut builder, &fb::LibraryListArgs {
        items: Some(items),
    });
...
pub fn serialize_library_list(libraries: &[ferrex_model::Library]) -> Vec<u8> {
    let mut builder = FlatBufferBuilder::with_capacity(1024 * libraries.len().max(1));
