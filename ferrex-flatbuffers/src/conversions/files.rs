...
pub fn build_media_file<'a>(
    ...
    let metadata = f.media_file_metadata.as_ref().map(|m| build_media_file_metadata(builder, m));

    fb::MediaFile::create(builder, &fb::MediaFileArgs {
        id: Some(&id),
...
fn build_media_file_metadata<'a>(
    ...
    let video_codec = m.video_codec.as_deref().map(|s| builder.create_string(s));
    let audio_codec = m.audio_codec.as_deref().map(|s| builder.create_string(s));
    let color_primaries = m.color_primaries.as_deref().map(|s| builder.create_string(s));
    let color_transfer = m.color_transfer.as_deref().map(|s| builder.create_string(s));
    let color_space = m.color_space.as_deref().map(|s| builder.create_string(s));

    fb::MediaFileMetadata::create(builder, &fb::MediaFileMetadataArgs {
