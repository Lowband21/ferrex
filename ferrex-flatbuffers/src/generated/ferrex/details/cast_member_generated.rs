// automatically generated by the FlatBuffers compiler, do not modify
// @generated
extern crate alloc;
use super::*;
pub enum CastMemberOffset {}
#[derive(Copy, Clone, PartialEq)]

pub struct CastMember<'a> {
    pub _tab: ::flatbuffers::Table<'a>,
}

impl<'a> ::flatbuffers::Follow<'a> for CastMember<'a> {
    type Inner = CastMember<'a>;
    #[inline]
    unsafe fn follow(buf: &'a [u8], loc: usize) -> Self::Inner {
        Self {
            _tab: unsafe { ::flatbuffers::Table::new(buf, loc) },
        }
    }
}

impl<'a> CastMember<'a> {
    pub const VT_ID: ::flatbuffers::VOffsetT = 4;
    pub const VT_PERSON_ID: ::flatbuffers::VOffsetT = 6;
    pub const VT_CREDIT_ID: ::flatbuffers::VOffsetT = 8;
    pub const VT_CAST_ID: ::flatbuffers::VOffsetT = 10;
    pub const VT_NAME: ::flatbuffers::VOffsetT = 12;
    pub const VT_ORIGINAL_NAME: ::flatbuffers::VOffsetT = 14;
    pub const VT_CHARACTER: ::flatbuffers::VOffsetT = 16;
    pub const VT_PROFILE_PATH: ::flatbuffers::VOffsetT = 18;
    pub const VT_ORDER: ::flatbuffers::VOffsetT = 20;
    pub const VT_GENDER: ::flatbuffers::VOffsetT = 22;
    pub const VT_KNOWN_FOR_DEPARTMENT: ::flatbuffers::VOffsetT = 24;
    pub const VT_ADULT: ::flatbuffers::VOffsetT = 26;
    pub const VT_POPULARITY: ::flatbuffers::VOffsetT = 28;
    pub const VT_ALSO_KNOWN_AS: ::flatbuffers::VOffsetT = 30;
    pub const VT_EXTERNAL_IDS: ::flatbuffers::VOffsetT = 32;
    pub const VT_IMAGE_SLOT: ::flatbuffers::VOffsetT = 34;
    pub const VT_IMAGE_ID: ::flatbuffers::VOffsetT = 36;

    #[inline]
    pub unsafe fn init_from_table(table: ::flatbuffers::Table<'a>) -> Self {
        CastMember { _tab: table }
    }
    #[allow(unused_mut)]
    pub fn create<
        'bldr: 'args,
        'args: 'mut_bldr,
        'mut_bldr,
        A: ::flatbuffers::Allocator + 'bldr,
    >(
        _fbb: &'mut_bldr mut ::flatbuffers::FlatBufferBuilder<'bldr, A>,
        args: &'args CastMemberArgs<'args>,
    ) -> ::flatbuffers::WIPOffset<CastMember<'bldr>> {
        let mut builder = CastMemberBuilder::new(_fbb);
        builder.add_cast_id(args.cast_id);
        builder.add_id(args.id);
        if let Some(x) = args.image_id {
            builder.add_image_id(x);
        }
        builder.add_image_slot(args.image_slot);
        if let Some(x) = args.external_ids {
            builder.add_external_ids(x);
        }
        if let Some(x) = args.also_known_as {
            builder.add_also_known_as(x);
        }
        builder.add_popularity(args.popularity);
        if let Some(x) = args.known_for_department {
            builder.add_known_for_department(x);
        }
        builder.add_order(args.order);
        if let Some(x) = args.profile_path {
            builder.add_profile_path(x);
        }
        if let Some(x) = args.character {
            builder.add_character(x);
        }
        if let Some(x) = args.original_name {
            builder.add_original_name(x);
        }
        if let Some(x) = args.name {
            builder.add_name(x);
        }
        if let Some(x) = args.credit_id {
            builder.add_credit_id(x);
        }
        if let Some(x) = args.person_id {
            builder.add_person_id(x);
        }
        builder.add_adult(args.adult);
        builder.add_gender(args.gender);
        builder.finish()
    }

    #[inline]
    pub fn id(&self) -> u64 {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe { self._tab.get::<u64>(CastMember::VT_ID, Some(0)).unwrap() }
    }
    #[inline]
    pub fn person_id(&self) -> Option<&'a super::ids::Uuid> {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab
                .get::<super::ids::Uuid>(CastMember::VT_PERSON_ID, None)
        }
    }
    #[inline]
    pub fn credit_id(&self) -> Option<&'a str> {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab.get::<::flatbuffers::ForwardsUOffset<&str>>(
                CastMember::VT_CREDIT_ID,
                None,
            )
        }
    }
    #[inline]
    pub fn cast_id(&self) -> u64 {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab
                .get::<u64>(CastMember::VT_CAST_ID, Some(0))
                .unwrap()
        }
    }
    #[inline]
    pub fn name(&self) -> &'a str {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab
                .get::<::flatbuffers::ForwardsUOffset<&str>>(
                    CastMember::VT_NAME,
                    None,
                )
                .unwrap()
        }
    }
    #[inline]
    pub fn original_name(&self) -> Option<&'a str> {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab.get::<::flatbuffers::ForwardsUOffset<&str>>(
                CastMember::VT_ORIGINAL_NAME,
                None,
            )
        }
    }
    #[inline]
    pub fn character(&self) -> Option<&'a str> {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab.get::<::flatbuffers::ForwardsUOffset<&str>>(
                CastMember::VT_CHARACTER,
                None,
            )
        }
    }
    #[inline]
    pub fn profile_path(&self) -> Option<&'a str> {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab.get::<::flatbuffers::ForwardsUOffset<&str>>(
                CastMember::VT_PROFILE_PATH,
                None,
            )
        }
    }
    #[inline]
    pub fn order(&self) -> u32 {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe { self._tab.get::<u32>(CastMember::VT_ORDER, Some(0)).unwrap() }
    }
    #[inline]
    pub fn gender(&self) -> u8 {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe { self._tab.get::<u8>(CastMember::VT_GENDER, Some(0)).unwrap() }
    }
    #[inline]
    pub fn known_for_department(&self) -> Option<&'a str> {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab.get::<::flatbuffers::ForwardsUOffset<&str>>(
                CastMember::VT_KNOWN_FOR_DEPARTMENT,
                None,
            )
        }
    }
    #[inline]
    pub fn adult(&self) -> bool {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab
                .get::<bool>(CastMember::VT_ADULT, Some(false))
                .unwrap()
        }
    }
    #[inline]
    pub fn popularity(&self) -> f32 {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab
                .get::<f32>(CastMember::VT_POPULARITY, Some(0.0))
                .unwrap()
        }
    }
    #[inline]
    pub fn also_known_as(
        &self,
    ) -> Option<
        ::flatbuffers::Vector<'a, ::flatbuffers::ForwardsUOffset<&'a str>>,
    > {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab.get::<::flatbuffers::ForwardsUOffset<
                ::flatbuffers::Vector<
                    'a,
                    ::flatbuffers::ForwardsUOffset<&'a str>,
                >,
            >>(CastMember::VT_ALSO_KNOWN_AS, None)
        }
    }
    #[inline]
    pub fn external_ids(&self) -> Option<PersonExternalIds<'a>> {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab
                .get::<::flatbuffers::ForwardsUOffset<PersonExternalIds>>(
                    CastMember::VT_EXTERNAL_IDS,
                    None,
                )
        }
    }
    #[inline]
    pub fn image_slot(&self) -> u32 {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab
                .get::<u32>(CastMember::VT_IMAGE_SLOT, Some(0))
                .unwrap()
        }
    }
    #[inline]
    pub fn image_id(&self) -> Option<&'a super::ids::Uuid> {
        // Safety:
        // Created from valid Table for this object
        // which contains a valid value in this slot
        unsafe {
            self._tab
                .get::<super::ids::Uuid>(CastMember::VT_IMAGE_ID, None)
        }
    }
}

impl ::flatbuffers::Verifiable for CastMember<'_> {
    #[inline]
    fn run_verifier(
        v: &mut ::flatbuffers::Verifier,
        pos: usize,
    ) -> Result<(), ::flatbuffers::InvalidFlatbuffer> {
        v.visit_table(pos)?
            .visit_field::<u64>("id", Self::VT_ID, false)?
            .visit_field::<super::ids::Uuid>(
                "person_id",
                Self::VT_PERSON_ID,
                false,
            )?
            .visit_field::<::flatbuffers::ForwardsUOffset<&str>>(
                "credit_id",
                Self::VT_CREDIT_ID,
                false,
            )?
            .visit_field::<u64>("cast_id", Self::VT_CAST_ID, false)?
            .visit_field::<::flatbuffers::ForwardsUOffset<&str>>(
                "name",
                Self::VT_NAME,
                true,
            )?
            .visit_field::<::flatbuffers::ForwardsUOffset<&str>>(
                "original_name",
                Self::VT_ORIGINAL_NAME,
                false,
            )?
            .visit_field::<::flatbuffers::ForwardsUOffset<&str>>(
                "character",
                Self::VT_CHARACTER,
                false,
            )?
            .visit_field::<::flatbuffers::ForwardsUOffset<&str>>(
                "profile_path",
                Self::VT_PROFILE_PATH,
                false,
            )?
            .visit_field::<u32>("order", Self::VT_ORDER, false)?
            .visit_field::<u8>("gender", Self::VT_GENDER, false)?
            .visit_field::<::flatbuffers::ForwardsUOffset<&str>>(
                "known_for_department",
                Self::VT_KNOWN_FOR_DEPARTMENT,
                false,
            )?
            .visit_field::<bool>("adult", Self::VT_ADULT, false)?
            .visit_field::<f32>("popularity", Self::VT_POPULARITY, false)?
            .visit_field::<::flatbuffers::ForwardsUOffset<
                ::flatbuffers::Vector<
                    '_,
                    ::flatbuffers::ForwardsUOffset<&'_ str>,
                >,
            >>("also_known_as", Self::VT_ALSO_KNOWN_AS, false)?
            .visit_field::<::flatbuffers::ForwardsUOffset<PersonExternalIds>>(
                "external_ids",
                Self::VT_EXTERNAL_IDS,
                false,
            )?
            .visit_field::<u32>("image_slot", Self::VT_IMAGE_SLOT, false)?
            .visit_field::<super::ids::Uuid>(
                "image_id",
                Self::VT_IMAGE_ID,
                false,
            )?
            .finish();
        Ok(())
    }
}
pub struct CastMemberArgs<'a> {
    pub id: u64,
    pub person_id: Option<&'a super::ids::Uuid>,
    pub credit_id: Option<::flatbuffers::WIPOffset<&'a str>>,
    pub cast_id: u64,
    pub name: Option<::flatbuffers::WIPOffset<&'a str>>,
    pub original_name: Option<::flatbuffers::WIPOffset<&'a str>>,
    pub character: Option<::flatbuffers::WIPOffset<&'a str>>,
    pub profile_path: Option<::flatbuffers::WIPOffset<&'a str>>,
    pub order: u32,
    pub gender: u8,
    pub known_for_department: Option<::flatbuffers::WIPOffset<&'a str>>,
    pub adult: bool,
    pub popularity: f32,
    pub also_known_as: Option<
        ::flatbuffers::WIPOffset<
            ::flatbuffers::Vector<'a, ::flatbuffers::ForwardsUOffset<&'a str>>,
        >,
    >,
    pub external_ids: Option<::flatbuffers::WIPOffset<PersonExternalIds<'a>>>,
    pub image_slot: u32,
    pub image_id: Option<&'a super::ids::Uuid>,
}
impl<'a> Default for CastMemberArgs<'a> {
    #[inline]
    fn default() -> Self {
        CastMemberArgs {
            id: 0,
            person_id: None,
            credit_id: None,
            cast_id: 0,
            name: None, // required field
            original_name: None,
            character: None,
            profile_path: None,
            order: 0,
            gender: 0,
            known_for_department: None,
            adult: false,
            popularity: 0.0,
            also_known_as: None,
            external_ids: None,
            image_slot: 0,
            image_id: None,
        }
    }
}

pub struct CastMemberBuilder<'a: 'b, 'b, A: ::flatbuffers::Allocator + 'a> {
    fbb_: &'b mut ::flatbuffers::FlatBufferBuilder<'a, A>,
    start_: ::flatbuffers::WIPOffset<::flatbuffers::TableUnfinishedWIPOffset>,
}
impl<'a: 'b, 'b, A: ::flatbuffers::Allocator + 'a>
    CastMemberBuilder<'a, 'b, A>
{
    #[inline]
    pub fn add_id(&mut self, id: u64) {
        self.fbb_.push_slot::<u64>(CastMember::VT_ID, id, 0);
    }
    #[inline]
    pub fn add_person_id(&mut self, person_id: &super::ids::Uuid) {
        self.fbb_.push_slot_always::<&super::ids::Uuid>(
            CastMember::VT_PERSON_ID,
            person_id,
        );
    }
    #[inline]
    pub fn add_credit_id(
        &mut self,
        credit_id: ::flatbuffers::WIPOffset<&'b str>,
    ) {
        self.fbb_.push_slot_always::<::flatbuffers::WIPOffset<_>>(
            CastMember::VT_CREDIT_ID,
            credit_id,
        );
    }
    #[inline]
    pub fn add_cast_id(&mut self, cast_id: u64) {
        self.fbb_
            .push_slot::<u64>(CastMember::VT_CAST_ID, cast_id, 0);
    }
    #[inline]
    pub fn add_name(&mut self, name: ::flatbuffers::WIPOffset<&'b str>) {
        self.fbb_.push_slot_always::<::flatbuffers::WIPOffset<_>>(
            CastMember::VT_NAME,
            name,
        );
    }
    #[inline]
    pub fn add_original_name(
        &mut self,
        original_name: ::flatbuffers::WIPOffset<&'b str>,
    ) {
        self.fbb_.push_slot_always::<::flatbuffers::WIPOffset<_>>(
            CastMember::VT_ORIGINAL_NAME,
            original_name,
        );
    }
    #[inline]
    pub fn add_character(
        &mut self,
        character: ::flatbuffers::WIPOffset<&'b str>,
    ) {
        self.fbb_.push_slot_always::<::flatbuffers::WIPOffset<_>>(
            CastMember::VT_CHARACTER,
            character,
        );
    }
    #[inline]
    pub fn add_profile_path(
        &mut self,
        profile_path: ::flatbuffers::WIPOffset<&'b str>,
    ) {
        self.fbb_.push_slot_always::<::flatbuffers::WIPOffset<_>>(
            CastMember::VT_PROFILE_PATH,
            profile_path,
        );
    }
    #[inline]
    pub fn add_order(&mut self, order: u32) {
        self.fbb_.push_slot::<u32>(CastMember::VT_ORDER, order, 0);
    }
    #[inline]
    pub fn add_gender(&mut self, gender: u8) {
        self.fbb_.push_slot::<u8>(CastMember::VT_GENDER, gender, 0);
    }
    #[inline]
    pub fn add_known_for_department(
        &mut self,
        known_for_department: ::flatbuffers::WIPOffset<&'b str>,
    ) {
        self.fbb_.push_slot_always::<::flatbuffers::WIPOffset<_>>(
            CastMember::VT_KNOWN_FOR_DEPARTMENT,
            known_for_department,
        );
    }
    #[inline]
    pub fn add_adult(&mut self, adult: bool) {
        self.fbb_
            .push_slot::<bool>(CastMember::VT_ADULT, adult, false);
    }
    #[inline]
    pub fn add_popularity(&mut self, popularity: f32) {
        self.fbb_
            .push_slot::<f32>(CastMember::VT_POPULARITY, popularity, 0.0);
    }
    #[inline]
    pub fn add_also_known_as(
        &mut self,
        also_known_as: ::flatbuffers::WIPOffset<
            ::flatbuffers::Vector<'b, ::flatbuffers::ForwardsUOffset<&'b str>>,
        >,
    ) {
        self.fbb_.push_slot_always::<::flatbuffers::WIPOffset<_>>(
            CastMember::VT_ALSO_KNOWN_AS,
            also_known_as,
        );
    }
    #[inline]
    pub fn add_external_ids(
        &mut self,
        external_ids: ::flatbuffers::WIPOffset<PersonExternalIds<'b>>,
    ) {
        self.fbb_
            .push_slot_always::<::flatbuffers::WIPOffset<PersonExternalIds>>(
                CastMember::VT_EXTERNAL_IDS,
                external_ids,
            );
    }
    #[inline]
    pub fn add_image_slot(&mut self, image_slot: u32) {
        self.fbb_
            .push_slot::<u32>(CastMember::VT_IMAGE_SLOT, image_slot, 0);
    }
    #[inline]
    pub fn add_image_id(&mut self, image_id: &super::ids::Uuid) {
        self.fbb_.push_slot_always::<&super::ids::Uuid>(
            CastMember::VT_IMAGE_ID,
            image_id,
        );
    }
    #[inline]
    pub fn new(
        _fbb: &'b mut ::flatbuffers::FlatBufferBuilder<'a, A>,
    ) -> CastMemberBuilder<'a, 'b, A> {
        let start = _fbb.start_table();
        CastMemberBuilder {
            fbb_: _fbb,
            start_: start,
        }
    }
    #[inline]
    pub fn finish(self) -> ::flatbuffers::WIPOffset<CastMember<'a>> {
        let o = self.fbb_.end_table(self.start_);
        self.fbb_.required(o, CastMember::VT_NAME, "name");
        ::flatbuffers::WIPOffset::new(o.value())
    }
}

impl ::core::fmt::Debug for CastMember<'_> {
    fn fmt(&self, f: &mut ::core::fmt::Formatter<'_>) -> ::core::fmt::Result {
        let mut ds = f.debug_struct("CastMember");
        ds.field("id", &self.id());
        ds.field("person_id", &self.person_id());
        ds.field("credit_id", &self.credit_id());
        ds.field("cast_id", &self.cast_id());
        ds.field("name", &self.name());
        ds.field("original_name", &self.original_name());
        ds.field("character", &self.character());
        ds.field("profile_path", &self.profile_path());
        ds.field("order", &self.order());
        ds.field("gender", &self.gender());
        ds.field("known_for_department", &self.known_for_department());
        ds.field("adult", &self.adult());
        ds.field("popularity", &self.popularity());
        ds.field("also_known_as", &self.also_known_as());
        ds.field("external_ids", &self.external_ids());
        ds.field("image_slot", &self.image_slot());
        ds.field("image_id", &self.image_id());
        ds.finish()
    }
}
