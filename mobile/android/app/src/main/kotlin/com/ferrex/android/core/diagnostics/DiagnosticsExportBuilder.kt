    ...
    private fun renderHumanSummary(manifest: DiagnosticsBundleManifest): String = DiagnosticsRedactor.redactText(
        buildString {
            ...
            manifest.snapshot.cache?.image?.let { image ->
                ...
                appendLine("imageManifestReady=${image.manifestStatus.readyCount}")
                appendLine("imageManifestPending=${image.manifestStatus.pendingCount}")
                appendLine("imageManifestFailed=${image.manifestStatus.failedCount}")
                appendLine("imageManifestStale=${image.manifestStatus.staleCount}")
                appendLine("imageManifestCorrupt=${image.manifestStatus.corruptCount}")
                appendLine("imageCoilBlobBytes=${image.coilBlobBytes}")
                appendLine("imageQuarantineFiles=${image.quarantineFileCount}")
                appendLine("imageQuarantineReasonFiles=${image.quarantineReasonFileCount}")
                appendLine("imageLastQuarantineEpochMs=${image.lastQuarantineEpochMs ?: "none"}")
                image.lastManifestBatch?.let { batch ->
                    appendLine("imageLastManifestOutcome=${batch.lastOutcome ?: "unknown"}")
                    appendLine("imageLastManifestKind=${batch.lastKind ?: "unknown"}")
                    appendLine("imageLastManifestRequestEpochMs=${batch.lastRequestEpochMs ?: "none"}")
                    appendLine("imageLastManifestSuccessEpochMs=${batch.lastSuccessEpochMs ?: "none"}")
                    appendLine("imageLastManifestFailureEpochMs=${batch.lastFailureEpochMs ?: "none"}")
                    appendLine("imageLastManifestRetryEpochMs=${batch.lastRetryEpochMs ?: "none"}")
                    appendLine("imageLastManifestRequestedKeys=${batch.lastRequestedKeyCount}")
                    appendLine("imageLastManifestResponseRecords=${batch.lastResponseRecordCount}")
                    appendLine("imageLastManifestReadyRecords=${batch.lastReadyCount}")
                    appendLine("imageLastManifestPendingRecords=${batch.lastPendingCount}")
                    appendLine("imageLastManifestFailedRecords=${batch.lastFailedCount}")
                    appendLine("imageLastManifestFailureKind=${batch.lastFailureKind ?: "none"}")
                    appendLine("imageLastManifestFailureClassification=${batch.lastFailureClassification ?: "none"}")
                    appendLine("imageLastManifestFailureHttpCode=${batch.lastFailureHttpCode ?: "none"}")
                }
            }
            appendLine()
            appendLine("Files:")
